...
local addon, ns = ...
AdvancedInterfaceOptions = LibStub("AceAddon-3.0"):NewAddon( "AdvancedInterfaceOptions", "AceConsole-3.0", "AceSerializer-3.0" )
AdvancedInterfaceOptions.Version = C_AddOns.GetAddOnMetadata( "AdvancedInterfaceOptions", "Version" )
AdvancedInterfaceOptions.Flavor = C_AddOns.GetAddOnMetadata( "AdvancedInterfaceOptions", "X-Flavor" ) or "Retail"
...
AdvancedInterfaceOptions.CurrentBuild = buildNum

if AdvancedInterfaceOptions.Version == ( "@" .. "project-version" .. "@" ) then
    AdvancedInterfaceOptions.Version = format( "Dev-%s (%s)", buildStr, date( "%Y%m%d" ) )
    AdvancedInterfaceOptions.IsDev = true
end

AdvancedInterfaceOptions.AllowSimCImports = true

AdvancedInterfaceOptions.IsRetail = function()
    return AdvancedInterfaceOptions.Flavor == "Retail"
end

AdvancedInterfaceOptions.IsWrath = function()
    return AdvancedInterfaceOptions.Flavor == "Wrath"
end

AdvancedInterfaceOptions.IsClassic = function()
    return AdvancedInterfaceOptions.IsWrath()
end

AdvancedInterfaceOptions.IsDragonflight = function()
    return buildNum >= 100000
end

AdvancedInterfaceOptions.IsTWW = function()
    return buildNum >= 110000
end

AdvancedInterfaceOptions.IsMidnight = function()
    return buildNum >= 120000
end

AdvancedInterfaceOptions.BuiltFor = 110200

AdvancedInterfaceOptions.GameBuild = buildStr

ns.PTR = buildNum > 110200
AdvancedInterfaceOptions.IsPTR = ns.PTR
...
do
    ...
    function AdvancedInterfaceOptions:ProfileCPU( name, func )
	...
	function AdvancedInterfaceOptions:ProfileFrame( name, f )
...
AdvancedInterfaceOptions.Class = {
    specs = {},
    num = 0,
...
AdvancedInterfaceOptions.Scripts = {
    DB = {},
    Channels = {},
    PackInfo = {},
}

AdvancedInterfaceOptions.State = {}
...
function AdvancedInterfaceOptions:Query( ... )
	local output = ns
...
function AdvancedInterfaceOptions:Run( ... )
	local n = select( "#", ... )
	local fn = select( n, ... )
...
function AdvancedInterfaceOptions:SetupDebug( display )
...
function AdvancedInterfaceOptions:Debug( ... )
    if not self.ActiveDebug then return end
...
function AdvancedInterfaceOptions:SaveDebugSnapshot( dispName )
    local snapped = false
	...
	for k, v in pairs( debug ) do
		if not dispName or dispName == k then
            ...
            insert( v.log, 1, "\n### Targets ###\n\ndetected_targets:  " .. ( AdvancedInterfaceOptions.TargetDebug or "no data" ) )
            insert( v.log, 1, self:GenerateProfile() )


            local performance
            local pInfo = AdvancedInterfaceOptionsEngine.threadUpdates
            ...
            if pInfo then
                performance = string.format( "\n\nPerformance\n"
                    ...
                    pInfo.updates, pInfo.updatesPerSec, pInfo.meanWorkTime, pInfo.meanClockTime, pInfo.meanFrames, pInfo.peakWorkTime, pInfo.peakClockTime, pInfo.peakFrames, GetFramerate() or 0, AdvancedInterfaceOptions.maxFrameTime or 0 )
            end
            ...
            local recs = AdvancedInterfaceOptions.DisplayPool[ dispName or state.display ].Recommendations
    ...
    if snapped then
        if AdvancedInterfaceOptions.DB.profile.screenshot and ( not hasScreenshotted or AdvancedInterfaceOptions.ManualSnapshot ) then
            Screenshot()
...
AdvancedInterfaceOptions.Snapshots = ns.snapshots



ns.Tooltip = CreateFrame( "GameTooltip", "AdvancedInterfaceOptionsTooltip", UIParent, "GameTooltipTemplate" )
AdvancedInterfaceOptions:ProfileFrame( "AdvancedInterfaceOptionsTooltip", ns.Tooltip )