...
AdvancedInterfaceOptions.unitIDs = unitIDs
AdvancedInterfaceOptions.npGUIDs = npGUIDs
AdvancedInterfaceOptions.npUnits = npUnits


function AdvancedInterfaceOptions:GetNameplateUnitForGUID( id )
    return npUnits[ id ]
end

function AdvancedInterfaceOptions:GetGUIDForNameplateUnit( unit )
    return npGUIDs[ unit ]
end

function AdvancedInterfaceOptions:GetUnitByGUID( id )
    for _, unit in ipairs( unitIDs ) do
        if UnitGUID( unit ) == id then return unit end
    end
end

function AdvancedInterfaceOptions:GetUnitByName( name )
    for _, unit in ipairs( unitIDs ) do
...
do
    ...
    function AdvancedInterfaceOptions:GetPetBasedTargetSpells()
        return petSpells[ myClass ]
    end

    function AdvancedInterfaceOptions:CanUsePetBasedTargetDetection()
        return petSpells[ myClass ] ~= nil
    end

    function AdvancedInterfaceOptions:HasPetBasedTargetSpell()
        return petSlot > 0
    end

    function AdvancedInterfaceOptions:GetPetBasedTargetSpell()
        return petAction > 0 and petAction or nil
    end

    function AdvancedInterfaceOptions:PetBasedTargetDetectionIsReady( skipRange )
    ...
    function AdvancedInterfaceOptions:SetupPetBasedTargetDetection()
        petAction = 0
        petSlot = 0
    ...
    function AdvancedInterfaceOptions:TargetIsNearPet( unit )
        return IsActionInRange( petSlot, unit )
    end

    function AdvancedInterfaceOptions:DumpPetBasedTargetInfo()
...
if AdvancedInterfaceOptions.IsDev then
...
do
    ...
    if not AdvancedInterfaceOptions.IsDragonflight() then
...
do
    ...
    function ns.getNumberTargets( forceUpdate )
        ...
        AdvancedInterfaceOptions.enemiesTotalHP = 0
        ...
        spec = spec and rawget( AdvancedInterfaceOptions.DB.profile.specs, spec )
        ...
        local checkPets = showNPs and spec and spec.petbased and AdvancedInterfaceOptions:PetBasedTargetDetectionIsReady()
        ...
        if spec then
            if checkPets or checkPlates then
                for unit, guid in pairs( npGUIDs ) do
                    ...
                    if (UnitAffectingCombat(unit) or AdvancedInterfaceOptions:CheakTargetIsJiaRen(unit, false)) and UnitExists(unit) and not UnitIsDead(unit) and UnitCanAttack("player", unit) and (UnitInPhase(unit) or AdvancedInterfaceOptions:CheakTargetIsJiaRen(unit, false)) and UnitHealth(unit) > 0 and (not inGroup or not FriendCheck(unit)) and (UnitIsPVP("player") or not UnitIsPlayer(unit)) then
                        ...
                        if excluded then
                            ...
                            if not excluded and checkPets then
                                excluded = not AdvancedInterfaceOptions:TargetIsNearPet( unit )
                        ...
                        if not excluded then
                            local rate, n = AdvancedInterfaceOptions:GetTTD( unit )
                            count = count + 1
                            counted[ guid ] = true
                    ...
                    if  counted[guid] == true and unit ~= nil then
                        AdvancedInterfaceOptions:countTotalTargetsHealth(unit, false)
                    end
                end
                ...
                for _, unit in ipairs( unitIDs ) do
                    ...
                    if guid and counted[ guid ] == nil then
                        ...
                        if (UnitAffectingCombat(unit) or AdvancedInterfaceOptions:CheakTargetIsJiaRen(unit, false)) and UnitExists(unit) and not UnitIsDead(unit) and UnitCanAttack("player", unit) and UnitAffectingCombat(unit) and (UnitInPhase(unit) or AdvancedInterfaceOptions:CheakTargetIsJiaRen(unit, false)) and UnitHealth(unit) > 1 and (not inGroup or not FriendCheck(unit)) and (UnitIsPVP("player") or not UnitIsPlayer(unit)) then
                            local excluded = not UnitIsUnit( unit, "target" )
                            ...
                            counted[ guid ] = counted[ guid ] or false
                            if  counted[guid] == true and unit then
                                AdvancedInterfaceOptions:countTotalTargetsHealth(unit, false)
                            end
                        end
                    end
        ...
        if not spec or spec.damage or not checkPets and not checkPlates then
            ...
            for guid, seen in pairs( db ) do
                if counted[ guid ] == nil then
                    ...
                    local unit = AdvancedInterfaceOptions:GetUnitByGUID( guid ) or UnitTokenFromGUID( guid )
        ...
        if targetGUID then
            if counted[ targetGUID ] == nil and UnitExists("target") and not UnitIsDead("target") and UnitCanAttack("player", "target") and (UnitInPhase("target") or AdvancedInterfaceOptions:CheakTargetIsJiaRen("target", true)) and (UnitIsPVP("player") or not UnitIsPlayer("target")) then
                count = count + 1
                ...
                AdvancedInterfaceOptions:countTotalTargetsHealth("target",true)
        ...
        if count ~= lastCount or stationary ~= lastStationary then
            ...
            if AdvancedInterfaceOptions:GetToggleState( "mode" ) == "reactive" then AdvancedInterfaceOptionsDisplayAOE:UpdateAlpha() end
        end

        if details then
            AdvancedInterfaceOptions.TargetDebug = details
        end
...
function AdvancedInterfaceOptions:GetNumTargets( forceUpdate )
...
ns.reportTargets = function()
    for k, v in pairs(targets) do
        AdvancedInterfaceOptions:Print("Saw " .. k .. " exactly " .. GetTime() - v .. " seconds ago.")
    end
end
...
function AdvancedInterfaceOptions:HasMinionID(id)
...
function AdvancedInterfaceOptions:DumpMinions()
    local o = ""
...
do
    ns.Audit = function( special )
        ...
        if AdvancedInterfaceOptionsEngine:IsThreadActive() then
            return
        end
        ...
        if not special and not AdvancedInterfaceOptions.DB.profile.enabled or not AdvancedInterfaceOptions:IsValidSpec() then
            return
        end

        AdvancedInterfaceOptions:ExpireTTDs()

        local now = GetTime()
        local spec = state.spec.id and rawget( AdvancedInterfaceOptions.DB.profile.specs, state.spec.id )
        ...
        for aura, targets in pairs( debuffs ) do
            ...
            for unit, entry in pairs( targets ) do
                ...
                elseif special == "combatExit" and not friendly then
                    -- AdvancedInterfaceOptions:Error( format( "Auditor removed an aura %d from %s after exiting combat.", aura, unit ) )
                    ns.trackDebuff( aura, unit )
                end
            end
        end
    ...
    AdvancedInterfaceOptions.AuditTimer = C_Timer.NewTicker( 1, ns.Audit )
end
AdvancedInterfaceOptions:ProfileCPU( "Audit", ns.Audit )


local IsAddOnLoaded, LoadAddOn = C_AddOns.IsAddOnLoaded, C_AddOns.LoadAddOn

function AdvancedInterfaceOptions:DumpDotInfo( aura )
    ...
    AdvancedInterfaceOptions:Print( "Current DoT Information at " .. GetTime() .. ( aura and ( " for " .. aura ) or "" ) .. ":" )
    DevTools_Dump( aura and debuffs[ aura ] or debuffs )
end

do
    -- New TTD, hopefully more aggressive and accurate than old TTD.
    AdvancedInterfaceOptions.TTD = AdvancedInterfaceOptions.TTD or {}
    local db = AdvancedInterfaceOptions.TTD
    ...
    function AdvancedInterfaceOptions:GetDeathClockByGUID( guid )
    ...
    function AdvancedInterfaceOptions:GetTTD( unit, isGUID )
        if state.target.is_dummy then return 180 end
    ...
    function AdvancedInterfaceOptions:GetTimeToPct( unit, percent )
    ...
    function AdvancedInterfaceOptions:GetTimeToPctByGUID( guid, percent )
        if percent >= 1 then
            percent = percent / 100
        end
    ...
    function AdvancedInterfaceOptions:GetGreatestTTD()
        if state.target.is_dummy then return 180 end
    ...
    function AdvancedInterfaceOptions:GetGreatestTimeToPct( percent )
        local time, validUnit, now = 0, false, GetTime()
    ...
    function AdvancedInterfaceOptions:GetLowestTTD()
        if state.target.is_dummy then return 180 end
    ...
    function AdvancedInterfaceOptions:GetNumTTDsWithin( x )
    ...
    AdvancedInterfaceOptions.GetNumTTDsBefore = AdvancedInterfaceOptions.GetNumTTDsWithin

    function AdvancedInterfaceOptions:GetNumTTDsAfter( x )
        local count = 0
        local dummy_override = state.target.is_dummy
    ...
    function AdvancedInterfaceOptions:GetNumTargetsAboveHealthPct( amount, inclusive, minTTD )
        local count, now = 0, GetTime()
    ...
    function AdvancedInterfaceOptions:GetNumTargetsBelowHealthPct( amount, inclusive, minTTD )
        amount = amount > 1 and ( amount / 100 ) or amount
        inclusive = inclusive or false
        minTTD = minTTD or 3
    ...
    function AdvancedInterfaceOptions:GetAddWaveTTD()
        if state.target.is_dummy then return 180 end
    ...
    function AdvancedInterfaceOptions:GetTTDInfo()
    ...
    function AdvancedInterfaceOptions:ExpireTTDs( all )
        local now = GetTime()
    ...
    AdvancedInterfaceOptions:ProfileCPU( "UpdateTTDs", UpdateTTDs )
