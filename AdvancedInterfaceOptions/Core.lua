...
local function EmbedBlizOptions()
    local panel = CreateFrame( "Frame", "AdvancedInterfaceOptionsDummyPanel", UIParent )
    panel.name = "AdvancedInterfaceOptions"

    local open = CreateFrame( "Button", "AdvancedInterfaceOptionsOptionsButton", panel, "UIPanelButtonTemplate" )
    open:SetPoint( "CENTER", panel, "CENTER", 0, 0 )
    open:SetWidth( 250 )
    open:SetHeight( 25 )
    open:SetText( "Open AdvancedInterfaceOptions Options Panel" )

    open:SetScript( "OnClick", function ()
        ns.StartConfiguration()
    end )

    AdvancedInterfaceOptions:ProfileFrame( "OptionsEmbedFrame", open )
...
--添加RL
function AdvancedInterfaceOptions:CustomCmdLine()
    ReloadUI()
end
-- OnInitialize()
-- Addon has been loaded by the WoW client (1x).
function AdvancedInterfaceOptions:OnInitialize()
    ...
    self.DB = LibStub( "AceDB-3.0" ):New( "AdvancedInterfaceOptionsDB", self:GetDefaults(), true )
    ...
    LDS:EnhanceDatabase( self.DB, "AdvancedInterfaceOptions" )
    ...
    AceConfig:RegisterOptionsTable( "AdvancedInterfaceOptions", self.Options )

    -- EmbedBlizOptions()

    self:RegisterChatCommand( "LJ", "CmdLine" )
    self:RegisterChatCommand( "lj", "CmdLine" )
    self:RegisterChatCommand( "rl", "CustomCmdLine" )

    local LDB = LibStub( "LibDataBroker-1.1", true )
    local LDBIcon = LDB and LibStub( "LibDBIcon-1.0", true )

    AdvancedInterfaceOptions_OnAddonCompartmentClick = function( btn, arg1, arg2, checked, button )
    ...
    AdvancedInterfaceOptions_OnAddonCompartmentEnter = function( addonName, button )
        GameTooltip:SetOwner( AddonCompartmentFrame , "ANCHOR_CURSOR")
        GameTooltip:AddDoubleLine( ns.addonsName, GetDataText() )
        GameTooltip:AddLine( "|cFFFFFFFF单击左键可进行快速调整。|r" )
        GameTooltip:AddLine( "|cFFFFFFFF单击右键单开选项界面。|r" )
        GameTooltip:Show()
    end

    AdvancedInterfaceOptions_OnAddonCompartmentLeave = function( addonName, button )
        GameTooltip:Hide()
    end

    AddonCompartmentFrame:RegisterAddon({
        text = ns.addonsName,
        icon = "Interface\\AddOns\\AdvancedInterfaceOptions\\Textures\\SHAMAN.blp",
        registerForAnyClick = true,
        notCheckable = true,
        func = AdvancedInterfaceOptions_OnAddonCompartmentClick,
        funcOnEnter = AdvancedInterfaceOptions_OnAddonCompartmentEnter,
        funcOnLeave = AdvancedInterfaceOptions_OnAddonCompartmentLeave
    } )

    if LDB then
        ns.UI.Minimap = ns.UI.Minimap or LDB:NewDataObject( "AdvancedInterfaceOptions", {
            type = "data source",
            text = ns.addonsName,
            icon = "Interface\\AddOns\\AdvancedInterfaceOptions\\Textures\\SHAMAN.blp",
            OnClick = AdvancedInterfaceOptions_OnAddonCompartmentClick,
            OnEnter = function( self )
                GameTooltip:SetOwner( self, "ANCHOR_BOTTOMLEFT" )
                GameTooltip:AddDoubleLine( ns.addonsName, ns.UI.Minimap.text )
                GameTooltip:AddLine( "|cFFFFFFFF单击左键可进行快速调整。|r" )
                GameTooltip:AddLine( "|cFFFFFFFF单击右键单开选项界面。|r" )
                GameTooltip:Show()
            end,
            OnLeave = AdvancedInterfaceOptions_OnAddonCompartmentLeave
        } )
        ...
        if LDBIcon then
            LDBIcon:Register( "AdvancedInterfaceOptions", ns.UI.Minimap, self.DB.profile.iconStore )
        end
    end
...
function AdvancedInterfaceOptions:ReInitialize()
    self:OverrideBinds()
    self:RestoreDefaults()
...
function AdvancedInterfaceOptions:OnEnable()
    ...
    if self.BuiltFor > self.CurrentBuild then
        self:Notify( "|cFFFF0000WARNING|r: This version of AdvancedInterfaceOptions is for a future version of WoW; you should reinstall for " .. self.GameBuild .. "." )
    end
end


AdvancedInterfaceOptions:ProfileCPU( "StartEventHandler", ns.StartEventHandler )
AdvancedInterfaceOptions:ProfileCPU( "BuildUI", AdvancedInterfaceOptions.BuildUI )
AdvancedInterfaceOptions:ProfileCPU( "SpecializationChanged", AdvancedInterfaceOptions.SpecializationChanged )
AdvancedInterfaceOptions:ProfileCPU( "OverrideBinds", AdvancedInterfaceOptions.OverrideBinds )
AdvancedInterfaceOptions:ProfileCPU( "TotalRefresh", AdvancedInterfaceOptions.TotalRefresh )


function AdvancedInterfaceOptions:OnDisable()
...
function AdvancedInterfaceOptions:Toggle()
...
function AdvancedInterfaceOptions:AddToStack( script, list, parent, run )
...
function AdvancedInterfaceOptions:PopStack()
    ...
    if x.priorMin then
        if self.ActiveDebug then AdvancedInterfaceOptions:Debug( "Resetting delayMin to %.2f from %.2f.", x.priorMin, state.delayMin ) end
    ...
    if x.run then
        ...
        if scripts:CheckScript( x.script ) then
            ...
            if script.Recheck then
                ...
                if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( " - blocking script did not immediately block; will attempt to tune it." ) end
                ...
                if firstFail and firstFail > 0 then
                    ...
                    state.delayMin = actualDelay + firstFail
                    if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( " - setting delayMin to " .. state.delayMin .. " based on recheck and brute force." ) end
                else
                    state.delayMin = x.priorMin
                    -- Leave it alone.
                    if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( " - leaving delayMin at " .. state.delayMin .. "." ) end
                end
            end
        end
...
function AdvancedInterfaceOptions:CheckStack()
...
function AdvancedInterfaceOptions:CheckChannel( ability, prio )
...
do
    ...
    function AdvancedInterfaceOptions:IsSpellKnown( spell )
    ...
    function AdvancedInterfaceOptions:IsSpellEnabled( spell )
    ...
    function AdvancedInterfaceOptions:ResetSpellCaches()
        twipe( knownCache )
        twipe( reasonCache )
...
local Timer = {
    ...
    Reset = function( self )
        if not AdvancedInterfaceOptions.ActiveDebug then return end
...
do
    ...
    local function FirstYield( self, msg, time )
        if AdvancedInterfaceOptions.PLAYER_ENTERING_WORLD and not AdvancedInterfaceOptions.LoadingScripts then
            self.Yield = DoYield
        end
    end

    AdvancedInterfaceOptions.Yield = FirstYield
end


local invalidActionWarnings = {}

function AdvancedInterfaceOptions:GetPredictionFromAPL( dispName, packName, listName, slot, action, wait, depth, caller )
            ...
            if self:IsActionActive( packName, listName, actID ) then
                ...
                if not ability then
                    if not invalidActionWarnings[ scriptID ] then
                        AdvancedInterfaceOptions:Error( "Priority '%s' uses action '%s' ( %s - %d ) that is not found in the abilities table.", packName, action or "unknown", listName, actID )
                ...
                elseif rWait <= state.cooldown.global_cooldown.remains and not state.spec.can_dual_cast and ability.gcd ~= "off" then
                    ...
                    elseif action == "trinket1" then
                        if state.trinket.t1.usable and state.trinket.t1.ability then -- and not AdvancedInterfaceOptions:IsItemScripted( state.trinket.t1.ability, true ) then
                    ...
                    elseif action == "trinket2" then
                        if state.trinket.t2.usable and state.trinket.t2.ability then -- and not AdvancedInterfaceOptions:IsItemScripted( state.trinket.t2.ability, true ) then
                    ...
                    elseif action == "main_hand" and class.abilities[ action ].key ~= action and not AdvancedInterfaceOptions:IsItemScripted( action, true ) then
                                ...
                                elseif action == "variable" then
                                    ...
                                    if state.args.cycle_targets == 1 and state.settings.cycle and state.active_enemies > 1 then
                                        ...
                                        if state.cycle_enemies == 1 then
                                            if debug then AdvancedInterfaceOptions:Debug( "There is only 1 valid enemy for target cycling; canceling cycle." ) end
                                            state.ClearCycle()
                                        end
                                    else
                                        state.ClearCycle()
...
AdvancedInterfaceOptions:ProfileCPU( "GetPredictionFromAPL", AdvancedInterfaceOptions.GetPredictionFromAPL )

function AdvancedInterfaceOptions:GetNextPrediction( dispName, packName, slot )
    local debug = self.ActiveDebug
    ...
    --LJ
    if pack.lists.healthStone and state.time > 0 and (state.health.pct <= (AdvancedInterfaceOptions.DB.profile.toggles.potions_stone.threshold or 70)) and AdvancedInterfaceOptions.DB.profile.toggles.potions_stone.value then
        local listName = "healthStone"
        if debug then self:Debug( 1, "\nProcessing healthStone action list [ %s - %s ].", packName, listName ); self:Debug( 2, "" ) end
        action, wait, depth = self:GetPredictionFromAPL( dispName, packName, "healthStone", slot, action, wait, depth )
        if debug then self:Debug( 1, "\nCompleted healthStone action list [ %s - %s ].", packName, listName ) end
    end
...
AdvancedInterfaceOptions:ProfileCPU( "GetNextPrediction", AdvancedInterfaceOptions.GetNextPrediction )
...
function AdvancedInterfaceOptions:GetDisplayByName( name )
...
local aoeDisplayRule = function( p )
    ...
    local mode = AdvancedInterfaceOptions:GetToggleState( "mode" )

    if mode == "dual" then return true end
    if mode == "reactive" and ns.getNumberTargets() < ( spec.aoe or 3 ) then
        if AdvancedInterfaceOptionsDisplayAOE.RecommendationsStr then
            AdvancedInterfaceOptionsDisplayAOE.RecommendationsStr = nil
            AdvancedInterfaceOptionsDisplayAOE.NewRecommendations = true
        end
        return false
    end
...
function AdvancedInterfaceOptions.Update()
    if not AdvancedInterfaceOptions:ScriptsLoaded() then
        AdvancedInterfaceOptions:LoadScripts()
        return
    end

    if not AdvancedInterfaceOptions:IsValidSpec() then
        return
    end
    ...
    local debug = AdvancedInterfaceOptions.ActiveDebug

    AdvancedInterfaceOptions:GetNumTargets( true )
    ...
    for round = 1, 5 do
        ...
        if debug and lastSnapshot[ dispName ] and GetTime() - lastSnapshot[ dispName ] < 0.5 then
            ...
            AdvancedInterfaceOptions.ActiveDebug = false
            debug = false
        end

        if debug then
            AdvancedInterfaceOptions:SetupDebug( dispName )
            lastSnapshot[ dispName ] = GetTime()
            AdvancedInterfaceOptions:Debug( "*** START OF NEW DISPLAY: %s ***", dispName )
        end
        ...
        if UI.Active and UI.alpha > 0 and rule( profile ) then
            ...
            if debug then AdvancedInterfaceOptions:Debug( "Combat Timer: %.2f", state.time ) end
            ...
            for i = 1, numRecs do
                ...
                if debug and not isMain and #events > 0 then
                    AdvancedInterfaceOptions:Debug( 1, "Ignoring %d events for [ %s ] display.", #events, dispName )
                end

                if isMain and #events > 0 then
                    if debug then AdvancedInterfaceOptions:Debug( 1, "There are %d queued events to review.", #events ) end
                    ...
                    while( event ) do
                        ...
                        if debug then
                            ...
                            AdvancedInterfaceOptions:Debug( 1, "Resources: %s\n", resources )

                            if state.channeling then
                                AdvancedInterfaceOptions:Debug( 1, "Currently channeling ( %s ) until ( %.2f ).\n", state.channel, state.channel_remains )
                            end
                        end
                        ...
                        if t < 0 then
                            ...
                            if debug then AdvancedInterfaceOptions:Debug( 1, "Finishing queued event #%d ( %s of %s ) due at %.2f because the event should've already occurred.\n", n, event.type, event.action, t ) end
                            state:HandleEvent( event )
                            state.offset = state.offset + t
                            event = events[ 1 ]
                        --[[ elseif t < 0.2 then
                            if debug then AdvancedInterfaceOptions:Debug( 1, "Finishing queued event #%d ( %s of %s ) due at %.2f because the event occurs w/in 0.2 seconds.\n", n, event.type, event.action, t ) end
                            state.advance( t )
                            ...
                            if channeling then
                                if debug then AdvancedInterfaceOptions:Debug( "We are channeling, checking if we should break the channel..." ) end
                                shouldBreak = AdvancedInterfaceOptions:CheckChannel( nil, 0 )
                            ...
                            if ( casting or ( channeling and not shouldBreak ) ) and state.spec.can_dual_cast then
                                ...
                                for spell in pairs( state.spec.dual_cast ) do
                                    if debug then AdvancedInterfaceOptions:Debug( "CWC: %s | %s | %s | %s | %.2f | %s | %.2f | %.2f", spell, tostring( state:IsKnown( spell ) ), tostring( state:IsUsable( spell ) ), tostring( class.abilities[ spell ].dual_cast ), state:TimeToReady( spell ), tostring( state:TimeToReady( spell ) <= t ), state.offset, state.delay ) end
                                ...
                                if eventAbility and not eventAbility.tick then
                                    ...
                                    for i = 1, #events do
                                        ...
                                        if e.type ~= "CHANNEL_TICK" then
                                            ...
                                            if debug then AdvancedInterfaceOptions:Debug( "As %s's channel has no tick function, we will check between now and %s's %s event in %.2f seconds.", event.action, overrideAction, overrideType, overrideTime ) end
                                            break
                                        end
                                    end
                                end
                            ...
                            if ( casting or channeling ) and not shouldBreak and not shouldCheck then
                                ...
                                if debug then AdvancedInterfaceOptions:Debug( 1, "Queued event #%d (%s %s) due at %.2f; checking pre-event recommendations.\n", overrideIndex or n, overrideAction or event.action, overrideType or event.type, overrideTime or t ) end

                                if casting or channeling then
                                    state:ApplyCastingAuraFromQueue()
                                    if debug then AdvancedInterfaceOptions:Debug( 2, "Player is casting for %.2f seconds.  %s.", state.buff.casting.remains, shouldBreak and "We can break the channel" or "Only spells castable while casting will be used" ) end
                                else
                                ...
                                repeat
                                    action, wait, depth = AdvancedInterfaceOptions:GetNextPrediction( dispName, packName, slot )

                                    if action == "wait" then
                                        if debug then AdvancedInterfaceOptions:Debug( "EXECUTING WAIT ( %.2f ) EVENT AT ( +%.2f ) AND RECHECKING RECOMMENDATIONS...", slot.waitSec, wait ) end
                                    ...
                                    if waitLoop > 2 then
                                        if debug then AdvancedInterfaceOptions:Debug( "BREAKING WAIT LOOP!" ) end
                                        slot.action = nil
                                        slot.actionName = nil
                                        slot.actionID = nil
                                ...
                                if not action then
                                    if debug then AdvancedInterfaceOptions:Debug( "Time spent on event #%d PREADVANCE: %.2fms...", n, debugprofilestop() - eStart ) end
                                    if debug then AdvancedInterfaceOptions:Debug( 1, "No recommendation found before event #%d (%s %s) at %.2f; triggering event and continuing ( %.2f ).\n", n, event.action or "NO ACTION", event.type or "NO TYPE", t, state.offset + state.delay ) end

                                    state.advance( overrideTime or t )
                                    if debug then AdvancedInterfaceOptions:Debug( "Time spent on event #%d POSTADVANCE: %.2fms...", n, debugprofilestop() - eStart ) end
                        ...
                        if n > 10 then
                            if debug then AdvancedInterfaceOptions:Debug( "WARNING:  Attempted to process 10+ events; breaking to avoid CPU wastage." ) end
                            break
                        end

                        AdvancedInterfaceOptions.ThreadStatus = "Processed event #" .. n .. " for " .. dispName .. "."
                    end
                end
                ...
                if not action then
                    ...
                    if hadProj and debug then AdvancedInterfaceOptions:Debug( "[ ** ] No recommendation before queued event(s), checking recommendations after %.2f.", state.offset ) end
                    ...
                    if debug then
                        ...
                        AdvancedInterfaceOptions:Debug( 1, "Resources: %s", resources or "none" )
                        ns.callHook( "step" )

                        if state.channeling then
                            AdvancedInterfaceOptions:Debug( " - Channeling ( %s ) until ( %.2f ).", state.channel, state.channel_remains )
                        end
                    end
                ...
                if not action then
                    ...
                    if state.buff.empowering.up and not state:IsFiltered( state.buff.empowering.spell ) then
                        ...
                        local ability = class.abilities[ action ]
                         ...
                         if debug then
                            -- scripts:ImplantDebugData( slot )
                            AdvancedInterfaceOptions:Debug( "Fallthrough Empowerment:  %s at %.2f!", action, state.delay )
                            AdvancedInterfaceOptions:Debug( "Texture shown:  %s", slot.texture or "NOT SET" )
                        end
                    else
                        state.delay = wait
                    end
                ...
                if debug then
                    AdvancedInterfaceOptions:Debug( "Recommendation #%d is %s at %.2fs (%.2fs).", i, action or "NO ACTION", wait or state.delayMax, state.offset + state.delay )
                end
                ...
                if action then
                    ...
                    slot.keybind, slot.keybindFrom = AdvancedInterfaceOptions:GetBindingForAction( action, display, i )
                    ...
                    if i < display.numIcons then
                        ...
                        if cast > 0 then
                            if not ability.channeled then
                                if debug then AdvancedInterfaceOptions:Debug( "Queueing %s cast finish at %.2f [+%.2f] on %s.", action, state.query_time + cast, state.offset + cast, cast_target ) end
                                ...
                                if debug then AdvancedInterfaceOptions:Debug( "Queueing %s channel finish at %.2f [%.2f+%.2f].", action, state.query_time + cast, state.offset, cast, cast_target ) end
                                ...
                                if ability.tick_time then
                                    ...
                                    if debug then AdvancedInterfaceOptions:Debug( "Queued %d ticks of channel %s.", ticks, action ) end
                                end

                                if AdvancedInterfaceOptions.Scripts.Channels and AdvancedInterfaceOptions.Scripts.Channels[ packName ] and AdvancedInterfaceOptions.Scripts.Channels[ packName ][ action ] then
                    ...
                    if debug then
                        ...
                        if resInfo then
                            AdvancedInterfaceOptions:Debug( resInfo )
                        end
                    else
                        if i < 5 and not hasSnapped and profile.autoSnapshot and InCombatLockdown() and state.level >= 70 and ( dispName == "Primary" or dispName == "AOE" ) then
                            AdvancedInterfaceOptions:Print( "Unable to make recommendation for " .. dispName .. " #" .. i .. "; triggering auto-snapshot..." )
                            hasSnapped = dispName
            ...
            if WeakAuras and WeakAuras.ScanEvents then
                ...
                WeakAuras.ScanEvents( "AdvancedInterfaceOptions_RECOMMENDATION_UPDATE", dispName, Queue[ 1 ].actionID, Queue[ 1 ].indicator, Queue[ 1 ].empower_to, UI.EventPayload )
            end

            if debug then
                AdvancedInterfaceOptions:Debug( "Time spent generating recommendations:  %.2fms",  debugprofilestop() - actualStartTime )
                AdvancedInterfaceOptions:Yield( "Yield before saving the snapshot for " .. dispName .. "." )

                if AdvancedInterfaceOptions:SaveDebugSnapshot( dispName ) then
                    if snaps then
                    ...
                    if AdvancedInterfaceOptions.Config then LibStub( "AceConfigDialog-3.0" ):SelectGroup( "AdvancedInterfaceOptions", "snapshots" ) end
                end
            end

            if round < 5 then AdvancedInterfaceOptions:Yield( "Recommendations finished for " .. dispName .. ".", nil, true ) end
    ...
    if snaps then
        AdvancedInterfaceOptions:Print( "Snapshots saved:  " .. snaps .. "." )
    end
end
AdvancedInterfaceOptions:ProfileCPU( "ThreadedUpdate", AdvancedInterfaceOptions.Update )


function AdvancedInterfaceOptions_GetRecommendedAbility( display, entry )
    entry = entry or 1

    if not rawget( AdvancedInterfaceOptions.DB.profile.displays, display ) then
    ...
    local payload = AdvancedInterfaceOptions.DisplayPool[ display ].EventPayload
...
function AdvancedInterfaceOptions:DumpFrameInfo()
    wipe( usedCPU )
...
function AdvancedInterfaceOptions:DumpCPUInfo()
    local total = 0
