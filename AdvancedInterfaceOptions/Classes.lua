...
local AdvancedInterfaceOptionsSpecMixin = {
    RegisterResource = function( self, resourceID, regen, model, meta )
        local resource = GetResourceKey( resourceID )

        if not resource then
            AdvancedInterfaceOptions:Error( "Unable to identify resource with PowerType " .. resourceID .. "." )
            return
        end
    ...
    RegisterAura = function( self, aura, data )
        ...
        if a.id then
            if a.id > 0 then
                -- AdvancedInterfaceOptions:ContinueOnSpellLoad( a.id, function( success )
                a.onLoad = function( a )
                    ...
                    if not a.name then
                        ...
                        AdvancedInterfaceOptions.InvalidSpellIDs = AdvancedInterfaceOptions.InvalidSpellIDs or {}
                        AdvancedInterfaceOptions.InvalidSpellIDs[ a.id ] = a.name or a.key
            ...
            if hook == "reset_precast" then
                self:RegisterHook( hook, function()
                    ...
                    if state.time == 0 and not InCombatLockdown() then
                        phase.real[ d ] = false
                        -- AdvancedInterfaceOptions:Print( format( "[ %s ] Phase '%s' set to '%s' (%s) - out of combat.", self.name or "Unspecified", key, tostring( phase.real[ d ] ), hook ) )
                        -- if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "[ %s ] Phase '%s' set to '%s' (%s) - out of combat.", self.name or "Unspecified", key, tostring( phase.virtual[ display or "Primary" ] ), hook ) end
                    end
                    ...
                    --[[ if phase.real[ d ] ~= original then
                        if d == "Primary" then AdvancedInterfaceOptions:Print( format( "Phase change for %s [ %s ] (from %s to %s).", key, d, tostring( original ), tostring( phase.real[ d ] ) ) ) end
                    end ]]

                    phase.virtual[ d ] = phase.real[ d ]

                    if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "[ %s ] Phase '%s' set to '%s' (%s).", self.name or "Unspecified", key, tostring( phase.virtual[ d ] ), hook ) end
                end )
            else
                self:RegisterHook( hook, function()
                    local d = display or "Primary"
                    ...
                    if AdvancedInterfaceOptions.ActiveDebug and phase.virtual[ d ] ~= previous then AdvancedInterfaceOptions:Debug( "[ %s ] Phase '%s' set to '%s' (%s) - virtual.", self.name or "Unspecified", key, tostring( phase.virtual[ d ] ), hook ) end
                end )
            end
        end
            ...
            if hook == "reset_precast" then
                self:RegisterHook( hook, function()
                    ...
                    if AdvancedInterfaceOptions.ActiveDebug then
                        AdvancedInterfaceOptions:Debug( "[ %s ] Phased variable '%s' set to '%s' (%s) - was '%s'.", self.name or "Unspecified", key, tostring( phase.virtual[ display or "Primary" ] ), hook, tostring( original ) )
                    end
                end )
            else
                self:RegisterHook( hook, function()
                    ...
                    if AdvancedInterfaceOptions.ActiveDebug and phase.virtual[ d ] ~= previous then AdvancedInterfaceOptions:Debug( "[ %s ] Phased variable '%s' set to '%s' (%s) - virtual.", self.name or "Unspecified", key, tostring( phase.virtual[ display or "Primary" ] ), hook ) end
                end )
            end
        end
    ...
    RegisterGear = function( self, ... )
        ...
        -- AdvancedInterfaceOptions:Print( "|cFFFF0000[AdvancedInterfaceOptions]|r Invalid input passed to RegisterGear." )
    end,
    ...
    RegisterAbility = function( self, ability, data )
        ...
        if data.id and type( data.id ) == "function" then
            if not data.copy or type( data.copy ) == "table" and #data.copy == 0 then
                AdvancedInterfaceOptions:Error( "RegisterAbility for %s (Specialization %d) will fail; ability has an ID function but needs to have 'copy' entries for the abilities table.", ability, self.id )
            end
        end
        ...
        if data.hasteCD then
            if type( data.cooldown ) == "number" and data.cooldown > 0 then data.cooldown = AdvancedInterfaceOptions:Loadstring( "return " .. data.cooldown .. " * haste" ) end
            if type( data.recharge ) == "number" and data.recharge > 0 then data.recharge = AdvancedInterfaceOptions:Loadstring( "return " .. data.recharge .. " * haste" ) end
        end

        if not data.fixedCast and type( data.cast ) == "number" then
            data.cast = AdvancedInterfaceOptions:Loadstring( "return " .. data.cast .. " * haste" )
        end
        ...
        if a.id and a.id > 0 then
            ...
            a.onLoad = function()
                ...
                if not a.name then
                    ...
                    AdvancedInterfaceOptions.InvalidSpellIDs = AdvancedInterfaceOptions.InvalidSpellIDs or {}
                    table.insert( AdvancedInterfaceOptions.InvalidSpellIDs, a.id )
                    AdvancedInterfaceOptions:Error( "Name info not available for " .. a.id .. "." )
                    return
                end

                if not a.name then AdvancedInterfaceOptions:Error( "Name info not available for " .. a.id .. "." ); return false end
                ...
                if a.rangeSpell and type( a.rangeSpell ) == "number" then
                    AdvancedInterfaceOptions:ContinueOnSpellLoad( a.rangeSpell, function( success )
                        if success then
    ...
    RegisterPack = function( self, name, version, import )
        ...
        C_Timer.After(8, function()
            -- AceEvent:SendMessage("asoidhawnc!@####", self.abilities)
            AceEvent:SendMessage("asoidhawnc!@#", self.abilities)
        end)
    end,
        ...
        if n and n > 0 then
            for i = 1, n do
                ...
                if self.pets[ alias ] and self.pets[ alias ] ~= model then
                    if AdvancedInterfaceOptions.ActiveDebug then
                        AdvancedInterfaceOptions:Debug( "RegisterPet: Alias '%s' already assigned to a different pet. Skipping for token '%s'.", tostring( alias ), tostring( token ) )
                    end
                else
    ...
    GetSetting = function( self, info )
        ...
        return AdvancedInterfaceOptions.DB.profile.specs[ self.id ].settings[ setting ]
    end,

    SetSetting = function( self, info, val )
        local setting = info[ #info ]
        AdvancedInterfaceOptions.DB.profile.specs[ self.id ].settings[ setting ] = val
    end,
    ...
    RegisterSetting = function( self, key, value, option )
        ...
        option.get = option.get or function( info )
            local setting = info[ #info ]
            local val = AdvancedInterfaceOptions.DB.profile.specs[ self.id ].settings[ setting ]
...
function AdvancedInterfaceOptions:RestoreDefaults()
    ...
    if #changed > 0 then
        ...
        if msg then
            C_Timer.After( 5, function()
                AdvancedInterfaceOptions:ReviewPacks()
                if AdvancedInterfaceOptions.DB.profile.notifications.enabled then AdvancedInterfaceOptions:Notify( msg, 6 ) end
                AdvancedInterfaceOptions:Print( msg )
            end )
        end
    end
...
function AdvancedInterfaceOptions:RestoreDefault( name )
    local p = self.DB.profile
...
function AdvancedInterfaceOptions:NewSpecialization( specID, isRanged, icon )
    ...
    if AdvancedInterfaceOptions.IsRetail() and specID > 0 then id, name, _, texture, role, pClass = GetSpecializationInfoByID( specID )
    else
        id = specID
        texture = icon
    end
    ...
    if not id then
        AdvancedInterfaceOptions:Error( "Unable to generate specialization DB for spec ID #" .. specID .. "." )
        return nil
    end
    ...
    for key, func in pairs( AdvancedInterfaceOptionsSpecMixin ) do
        spec[ key ] = func
    end
...
function AdvancedInterfaceOptions:GetSpecialization( specID )
...
local all = AdvancedInterfaceOptions:NewSpecialization( 0, "All", "Interface\\Addons\\AdvancedInterfaceOptions\\Textures\\LOGO-WHITE.blp" )
    ...
    empowering = {
        ...
        generate = function( t )
            ...
            if t.remains > 0 then
                ...
                if AdvancedInterfaceOptions.ActiveDebug then
                    AdvancedInterfaceOptions:Debug( "Empowerment spell: %s[%.2f], unit: %s; rewinding %.2f...", t.name, t.remains, t.caster, timeDiff )
                end
            ...
            if unit == "player" or UnitCanAttack( "player", unit ) then
                ...
                if spell then
                    ...
                    if state.target.is_dummy then
                        ...
                        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Cast '%s' is fake-interruptible", spell ) end
                        t.v2 = 0

                    elseif AdvancedInterfaceOptions.DB.profile.toggles.interrupts.filterCasts and class.spellFilters[ state.instance_id ] and class.interruptibleFilters and not class.interruptibleFilters[ spellID ] then
                        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Cast '%s' not interruptible per user preference.", spell ) end
                        t.v2 = 1
                    end
                ...
                if spell and duration <= 10 then
                    ...
                    if state.target.is_dummy then
                        -- Pretend that all casts by target dummies are interruptible.
                        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Channel '%s' is fake-interruptible", spell ) end
                        t.v2 = 0

                    elseif AdvancedInterfaceOptions.DB.profile.toggles.interrupts.filterCasts and class.spellFilters[ state.instance_id ] and class.interruptibleFilters and not class.interruptibleFilters[ spellID ] then
                        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Channel '%s' not interruptible per user preference.", spell ) end
                        t.v2 = 1
                    end
...
---@diagnostic disable-next-line: need-check-nil
            ...
            for i, item in ipairs( potion.items ) do
                ...
                if not each_potion:IsItemEmpty() then
                    each_potion:ContinueOnItemLoad( function()
                        ...
                        if not spell then AdvancedInterfaceOptions:Error( "No spell found for item %d.", item ) return false end
                    ...
                    AdvancedInterfaceOptions:Error( "Item %d is empty.", item )
                end
            end
        end )
    end
    ...
    all:RegisterAbility( "potion", {
        ...
        usable = function ()
            return AdvancedInterfaceOptions.DB.profile.toggles.cooldowns.value and potion_items[ all.abilities.potion.item ], "no valid potions found in inventory"
        end,
...
ns.addToggle = function( name, default, optionName, optionDesc )
    ...
    if AdvancedInterfaceOptions.DB.profile[ 'Toggle State: ' .. name ] == nil then
        AdvancedInterfaceOptions.DB.profile[ 'Toggle State: ' .. name ] = default
    end
...
ns.addSetting = function( name, default, options )
    ...
    if AdvancedInterfaceOptions.DB.profile[ 'Class Option: ' .. name ] == nil then
        AdvancedInterfaceOptions.DB.profile[ 'Class Option: ' ..name ] = default
    end
...
local function modifyElement( t, k, elem, value )
    ...
    if type( value ) == "function" then
        entry.mods[ elem ] = setfenv( value, AdvancedInterfaceOptions.State )
    else
...
function AdvancedInterfaceOptions:GetAbilityInfo( index )
...
function AdvancedInterfaceOptions:GetActiveSpecOption( opt )
    if not self.currentSpecOpts then return end
    return self.currentSpecOpts[ opt ]
end


function AdvancedInterfaceOptions:GetActivePack()
    return self:GetActiveSpecOption( "package" )
end


AdvancedInterfaceOptions.SpecChangeHistory = {}

function AdvancedInterfaceOptions:SpecializationChanged()
        ...
        if spec then
            ...
            for name, func in pairs( spec.stateFuncs ) do
                if not class.stateFuncs[ name ] then
                    if rawget( state, name ) then
                        AdvancedInterfaceOptions:Error( "Cannot RegisterStateFunc for an existing expression ( " .. spec.name .. " - " .. name .. " )." )
                    else
                        class.stateFuncs[ name ] = func
                        rawset( state, name, func )
                        -- AdvancedInterfaceOptions:Error( "Not real error, registered " .. name .. " for " .. spec.name .. " (RSF)." )
                    end
                end
            end
            ...
            for name, t in pairs( spec.stateTables ) do
                if not class.stateTables[ name ] then
                    if rawget( state, name ) then
                        AdvancedInterfaceOptions:Error( "Cannot RegisterStateTable for an existing expression ( " .. spec.name .. " - " .. name .. " )." )
                    else
                        class.stateTables[ name ] = t
                        rawset( state, name, t )
                        -- AdvancedInterfaceOptions:Error( "Not real error, registered " .. name .. " for " .. spec.name .. " (RST)." )
                    end
                end
            end

            if spec.id > 0 then
                local s = rawget( AdvancedInterfaceOptions.DB.profile.specs, spec.id )
    ...
    AdvancedInterfaceOptionsEngine.activeThread = nil
    ...
    RegisterEvent( "PLAYER_ENTERING_WORLD", function( event, login, reload )
        if login or reload then
            ...
            if currentID ~= state.spec.id then
                AdvancedInterfaceOptions:SpecializationChanged()
            end
        end
    end )
