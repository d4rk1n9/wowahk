...
local mt_trinket = {
    __index = function( t, k )
        ...
        AdvancedInterfaceOptions:Error( "Unknown trinket expression '%s' in %s.", k, state.this_id )
        return k
    end
}
...
if AdvancedInterfaceOptions.Version:match( "^Dev" ) then
    state.print = print
else
...
state.class = AdvancedInterfaceOptions.Class
...
do
    local cycle = {}
    local debug = function( ... ) if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( ... ) end end
...
local function applyDebuff( unit, aura, duration, stacks, value, noPandemic )
    ...
    if state.cycle then
        if duration == 0 then
            if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Removed an application of '%s' while target-cycling.", aura ) end
            state.active_dot[ aura ] = state.active_dot[ aura ] - 1
            if model.key ~= aura then state.active_dot[ model.key ] = state.active_dot[ aura ] end
        else
            if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Added an application of '%s' while target-cycling.", aura ) end
            state.active_dot[ aura ] = state.active_dot[ aura ] + 1
            if model.key ~= aura then state.active_dot[ model.key ] = state.active_dot[ aura ] end
        end
        return
...
local function timeToInterrupt()
    ...
    if UnitIsPlayer("target") then
        return 3600 -- Effectively disables interrupting players by returning a very high time.
    end
    local casting = state.debuff.casting
    if casting.down or casting.v2 == 1 then return 3600 end
    if casting.v3 == 1 then return 0.25 end
    return max( 0, casting.remains - ( AdvancedInterfaceOptions.DB.profile.toggles.interrupts.castRemainingThreshold or 0.25 ) )
end
...
function state.stopChanneling( reset, action )
    if not reset then
        ...
        if spell then
            if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Breaking channel of %s.", spell ) end
...
state.spell_targets = setmetatable( {}, {
    __index = function( t, k )
        ...
        if ability.max_ttd then n = min( n, AdvancedInterfaceOptions:GetNumTTDsBefore( ability.max_ttd + state.offset + state.delay ) ) end
        if ability.min_ttd then n = min( n, AdvancedInterfaceOptions:GetNumTTDsAfter( ability.min_ttd + state.offset + state.delay ) ) end
...
do
        ...
        for i = 1, n do
            ...
            if type( x ) == "number" then
                if x > 0 and x >= state.delayMin and x <= state.delayMax then
                    t[ x ] = true
                -- elseif AdvancedInterfaceOptions.ActiveDebug and x < 60 then
                --    AdvancedInterfaceOptions:Debug( "Excluded %.2f recheck time as it is outside our constraints ( %.2f - %.2f ).", x, state.delayMin or -1, state.delayMax or -1 )
                end
            end
        end
    end
    ...
    function state.recheck( ability, script, stack, block )
        ...
        -- if AdvancedInterfaceOptions.ActiveDebug then table.insert( steps, debugprofilestop() ) end
        ...
        for k, v in pairs( workTable ) do
            -- if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "%s - %s", tostring( k ), tostring( v ) ) end
            times[ #times + 1 ] = k
        end
    ...
    mt_state = {
        __index = function( t, k )
            ...
            elseif k == "fight_remains" or k == "interpolated_fight_remains" or k == "time_to_die" then
                ...
                if not n then AdvancedInterfaceOptions:Error( "longest_ttd was nil, GetGreatestTTD is " .. ( AdvancedInterfaceOptions:GetGreatestTTD() or "nil" ) .. "." ) end
                return max( 1, t.longest_ttd - ( t.offset + t.delay ) )
            elseif k == "index" then t[k] = 0
            elseif k == "longest_ttd" then t[k] = AdvancedInterfaceOptions:GetGreatestTTD()
            elseif k == "now" then t[k] = GetTime()
            elseif k == "offset" then t[k] = 0
            elseif k == "query_time" then  return t.now + t.offset + t.delay
            elseif k == "shortest_ttd" then t[k] = AdvancedInterfaceOptions:GetLowestTTD()
            elseif k == "time" then
            ...
            elseif type(k) == "string" and k:sub(1, 12) == "time_to_pct_" then
                ...
                return AdvancedInterfaceOptions:GetGreatestTimeToPct( percent ) - ( t.offset + t.delay )
            ...
            elseif k == "cycle_enemies" then
                ...
                targets = targets - AdvancedInterfaceOptions:GetNumTTDsBefore( minTTD )

                if maxTTD then
                    targets = targets - AdvancedInterfaceOptions:GetNumTTDsAfter( maxTTD )
                end
                ...
                -- if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "cycle min:%.2f, max:%.2f, ae:%d, before:%d, after:%d, cycle_enemies:%d", minTTD or 0, maxTTD or 0, t.active_enemies, minTTD and AdvancedInterfaceOptions:GetNumTTDsBefore( minTTD ) or 0, maxTTD and AdvancedInterfaceOptions:GetNumTTDsAfter( maxTTD ) or 0, max( 1, targets ) ) end
            ...
            elseif k == "fight_style" then return state.fight_style
            elseif k == "DungeonSlice" or k == "dungeonSlice" or k == "dungeonroute" then return true
            elseif k == "patchwerk" then return true
            ...
            if k ~= "scriptID" and not ( logged_state_errors[ t.scriptID ] and logged_state_errors[ t.scriptID ][ k ] ) then
                AdvancedInterfaceOptions:Error( "Unknown key '" .. k .. "' in emulated environment for [ " .. t.scriptID .. " : " .. t.this_action .. " ].\n\n" .. debugstack() )
            ...
            if not rawget( t, "real_pet" ) then
                ...
                if key == nil then
                    if AdvancedInterfaceOptions.ActiveDebug then
                        AdvancedInterfaceOptions:Debug( "Failed to resolve pet ID %s to any registered pet.", tostring( petID ) )
                    end
                end
            end
...
local mt_toggle = {
    __index = function( t, k )
        ...
        local db = AdvancedInterfaceOptions.DB
    ...
    mt_target = {
        __index = function( t, k )
            ...
            elseif k == "real_ttd" or k == "true_ttd" then
                t[k] = AdvancedInterfaceOptions:GetTTD( "target" )
            ...
            elseif k:sub(1, 12) == "time_to_pct_" then
                local percent = tonumber( k:sub( 13 ) ) or 0
                return AdvancedInterfaceOptions:GetTimeToPct( "target", percent ) - ( state.offset + state.delay )
            ...
            if k == "duration" or k == "expires" or k == "charge" or k == "recharge_began" then
                ...
                if id > 0 then
                    ...
                    if state.debuff.voidbinding.up and modRate and modRate ~= 1 then
                        ...
                        if extraTime > 0 then
                            if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Extending '%s' remaining cooldown by %.2f because the cooldown exceeds Voidbinding's remaining time by %.2f.", ability.key, ( extraTime * 0.3 ), extraTime ) end                            duration = duration + ( extraTime * 0.3 )
                        end
                    end
                end
...
do
    ...
    mt_buffs = {
        ...
        __index = function( t, k )
            ...
            if not aura then
                if AdvancedInterfaceOptions.PLAYER_ENTERING_WORLD and not buffs_warned[ k ] then
                    AdvancedInterfaceOptions:Error( "Unknown buff in [" .. state.scriptID .. "]: " .. k .. "\n\n" .. debugstack() )
                    buffs_warned[ k ] = true
                end
                return unknown_buff
            end
...
local mt_talents = {
    __index = function( t, k )
        if class.talents[ k ] == nil and not logged_talent_errors[ k ] and #class.specs > 1 then
            AdvancedInterfaceOptions:Error( "Unknown talent in [ " .. state.scriptID .. " ]: " .. k .. "\n\n" .. debugstack() )
...
AdvancedInterfaceOptions.mt_default_totem = mt_default_totem
...
do
    ...
    state.variable = setmetatable( {
        ...
        __index = function( t, var )
            ...
            if AdvancedInterfaceOptions.LoadingScripts then
                return defaultValue
            end

            if not db[ var ] then
                if debug then AdvancedInterfaceOptions:Debug( "No such variable '%s'.", var ) end
                AdvancedInterfaceOptions:Error( "Variable '%s' referenced in %s but is undefined.", var, state.scriptID )
                return defaultValue
            end
                ...
                if pathState[ currPath ] then
                    ...
                    if cache[ var ][ pathKey ] ~= nil then
                        ...
                        if debug then
                            ...
                            AdvancedInterfaceOptions:Debug( var .. " #" .. i .. " [" .. scriptID .. "]; conditions = " .. conditions .. "\n - value = " .. valueString )
                        end
                        state.variable[ var ] = value
            ...
            --[[ if debug then
                AdvancedInterfaceOptions:Debug( "%s Result = %s.", var, tostring( value ) )
            end ]]
    ...
    mt_debuffs = {
        ...
        __index = function( t, k )
            ...
            if aura then
                ...
                if AdvancedInterfaceOptions.PLAYER_ENTERING_WORLD and not debuffs_warned[ k ] then
                    AdvancedInterfaceOptions:Error( "WARNING: Unknown debuff in [" .. ( state.scriptID or "unknown" ) .. "]: " .. k .. "\n\n" .. debugstack() )
...
do
    ...
    AdvancedInterfaceOptions.StoreMatchingAuras = state.StoreMatchingAuras
    ...
    AdvancedInterfaceOptions.ScrapeUnitAuras = state.ScrapeUnitAuras

    AdvancedInterfaceOptions:ProfileCPU( "ScrapeUnitAuras", state.ScrapeUnitAuras )

    AdvancedInterfaceOptions.AuraDB = ns.auras
end
...
do
    ...
    function state:QueueEvent( action, start, time, type, target, real )
        ...
        if action and start and time and type then
            ...
            if AdvancedInterfaceOptions.ActiveDebug and not real then AdvancedInterfaceOptions:Debug( "Queued %s from %.2f to %.2f (%s).", action, start, time, type ) end
        end
    end

    AdvancedInterfaceOptions:ProfileCPU( "QueueEvent", state.QueueEvent )
    ...
    function state:QueueAuraEvent( action, func, time, eType, data )
        ...
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Queued %s %s at +%.2f.", action, eType, time - state.query_time ) end
    end
    ...
    function state:RemoveAuraEvent( action, eType )
        ...
        AdvancedInterfaceOptions:Debug( "Trying to remove %s %s from queue.", action, eType )
        ...
        for i = 1, #queue do
            ...
            if e.action == action and e.type == eType then
                ...
                AdvancedInterfaceOptions:Debug( "Removed #%d from queue.", i )
                break
            end
        end
    end
    ...
    function state:RemoveEvent( e, real )
        ...
        AdvancedInterfaceOptions:Debug( "Trying to remove %s %s from queue.", ( e.action or "NO_ACTION" ), ( e.type or "NO_TYPE" ) )

        for i = #queue, 1, -1 do
            if queue[ i ] == e then
                AdvancedInterfaceOptions:Debug( "Removing %d from queue.", i )
                RecycleEvent( queue, i )
    ...
    AdvancedInterfaceOptions:ProfileCPU( "RemoveEvent", state.RemoveEvent )
    ...
    function state:HandleEvent( e )
        ...
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "\nHandling %s at %.2f (%s).", action, e.time, e.type ) end
        ...
        if e.type == "CAST_FINISH" then
            ...
            if e.target and e.target ~= self.target.unit then
                if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Using ability on a different target." ) end
    ...
    AdvancedInterfaceOptions:ProfileCPU( "HandleEvent", state.HandleEvent )
...
function state:RunHandler( key, noStart )
    ...
    if AdvancedInterfaceOptions.ActiveDebug then
        AdvancedInterfaceOptions:Debug( "Running %s at %.2f.", key, self.query_time )
    end
...
do
    ...
    function state.reset( dispName, full )
        ...
        local displayFrame = _G[ "AdvancedInterfaceOptionsDisplay" .. dispName ]
        ...
        -- AdvancedInterfaceOptions:Yield( "Reset Post-States" )
        ...
        if state.time == 0 and InCombatLockdown() then
            ...
            if a and a.startsCombat and state.now - a.lastCast < 1 then
                ...
                if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( format( "Starting combat based on %s cast; time is now: %.2f.", state.player.lastcast, state.time ) ) end
            end
        end

        local foundResource = false

        -- AdvancedInterfaceOptions:Yield( "Reset Pre-Powers" )
        ...
        -- AdvancedInterfaceOptions:Yield( "Reset Post-Powers" )
        ...
        -- AdvancedInterfaceOptions:Yield( "Reset Pre-Cast Hook" )
        ns.callHook( "reset_precast" )
        -- AdvancedInterfaceOptions:Yield( "Reset Pre-Casting" )
        ...
        if casting and cast_time > 0 then
            ...
            if ability then
                ...
                destGUID  = AdvancedInterfaceOptions:GetMacroCastTarget( ability.key, state.buff.casting.applied, "RESET" ) or state.target.unit
            end
            ...
            elseif not state:IsChanneling() and channeled then
                ...
                if ability then
                    ...
                    if tick_time and tick_time > 0 then
                        ...
                        local expires = state.buff.casting.expires
            ...
            if dispName == "Primary" or dispName == "AOE" then
                ...
                if delay > 0 then
                    if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Advancing by %.2f per GCD or cast or channel or reset_postcast value.", delay ) end
                    state.advance( delay )
                end
            end
        ...
        -- AdvancedInterfaceOptions:Yield( "Reset Post-Casting" )
...
AdvancedInterfaceOptions:ProfileCPU( "state.reset", state.reset )
...
function state:StartCombat()
    if self.time == 0 then
        ...
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( format( "Starting combat at %.2f -- time is %.2f.", self.false_start, self.time ) ) end
    end
...
function state.advance( time )
    ...
    while( event ) do
        if event.time <= state.query_time + time then
            ...
            if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "While advancing by %.2f to %.2f, %s %s occurred at %.2f.", time, realOffset + time, event.action, event.type, state.offset ) end
...
do
    ...
    function AdvancedInterfaceOptions:PlaceHold( action, combat, verbose )
        if not action then return end
        ...
        if ability then
            ...
            if verbose then AdvancedInterfaceOptions:Print( class.abilities[ ability.key ].name .. " placed on hold" .. ( combat and " until end of combat." or "." ) ) end
            AdvancedInterfaceOptions:ForceUpdate( "AdvancedInterfaceOptions_HOLD_APPLIED" )
        end
    end

    function AdvancedInterfaceOptions:RemoveHold( action, verbose )
        if not action then return end
        ...
        if ability and state.holds[ ability.key ] then
            ...
            if verbose then AdvancedInterfaceOptions:Print( class.abilities[ ability.key ].name .. " hold removed." ) end
            AdvancedInterfaceOptions:ForceUpdate( "AdvancedInterfaceOptions_HOLD_REMOVED" )
        end
    end

    function AdvancedInterfaceOptions:ToggleHold( action, combat, verbose )
    ...
    function AdvancedInterfaceOptions:IsHeld( action )
    ...
    function AdvancedInterfaceOptions:ReleaseHolds( combat )
        ...
        if holdRemoved then AdvancedInterfaceOptions:ForceUpdate( "AdvancedInterfaceOptions_COMBAT_HOLD_REMOVED" ) end
    end
end
...
do
    ...
    function state:IsDisabled( spell, strict )
        ...
        if ability.item and ability.toggle ~= "potions" then
            ...
            if sp and (not AdvancedInterfaceOptions.DB.profile.toggles.enable_items.value) then
                return true, "preference - spec disables gear/items"
            end
        end
                
        -- LJ 技能添加距离设定
        if option.distance_check and option.distance_check > 0 then
            if (state.target.distance or 0) > option.distance_check then
                return true
            end
        end
        --血量判定
        if option.tar_hp_check and option.tar_hp_check > 0 then
            if (state.target.health.pct or 0) <= option.tar_hp_check then
                return true
            end
        end
        ...
        if dotCap and dotCap > 0 and class.auras[ spell ] then
            ...
            if AdvancedInterfaceOptions.ActiveDebug then
                if activeDot < dotCap then
                    AdvancedInterfaceOptions:Debug( "    %s: current applications fewer than maximum applications [%d/%d].", spell, activeDot, dotCap )
                elseif aura and aura.refreshable then
                    AdvancedInterfaceOptions:Debug( "    %s: target refreshable despite maximum applications [%d/%d]", spell, activeDot, dotCap )
                end
            end
        end
...
function state:TimeToReady( action, pool )
    ...
    if ability.timeToReadyOverride then
        ...
        if override ~= nil then
            ...
            if AdvancedInterfaceOptions.ActiveDebug then
                AdvancedInterfaceOptions:Debug( "%s has a timeToReadyOverride. Returning %.2f.", action, override )
            end
    ...
    if z and z > wait then
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "ICD is " .. z .. ", last cast was " .. lastCast .. ", remaining CD: " .. max( 0, lastCast + z - now ) ) end
        wait = z
    end
    ...
    if self.time > 0 and lastCast > max( self.combat, self.false_start ) and line_cd and type( line_cd ) == "number" then
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Line CD is " .. line_cd .. ", last cast was " .. lastCast .. ", remaining CD: " .. max( 0, lastCast + line_cd - now ) ) end
...
function state:IsReady( action )
    ...
    if not ability then
        AdvancedInterfaceOptions:Error( "Failed state:IsReady( " .. ( action or "BLANK" ) .. " )." )
        return false
    end
...
function state:ClashOffset( action )
    ...
    local spec = rawget( AdvancedInterfaceOptions.DB.profile.specs, state.spec.id )
