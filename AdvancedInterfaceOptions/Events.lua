...
AdvancedInterfaceOptions:ProfileFrame( "GeneralEvents", events )
local handlers = {}
local unitHandlers = {}
...
function AdvancedInterfaceOptions:GetActiveDisplays()
    return activeDisplays
end


local handlerCount = {}
AdvancedInterfaceOptions.ECount = handlerCount
AdvancedInterfaceOptions.IC = itemCallbacks

local eventData = {}
AdvancedInterfaceOptions.EData = eventData
...
function ns.StartEventHandler()
    ...
    events:SetScript( "OnUpdate", function( self, elapsed )
        if AdvancedInterfaceOptions.PendingSpecializationChange then
            AdvancedInterfaceOptions:SpecializationChanged()
            AdvancedInterfaceOptions.PendingSpecializationChange = false
    ...
    AdvancedInterfaceOptions:RunSpellCallbacks()
end
...
AdvancedInterfaceOptions.EventSources = {}
...
ns.RegisterEvent = function( event, handler )
    ...
    AdvancedInterfaceOptions:ProfileCPU( key, handler )

    local stack = debugstack(2)
    local file, line = stack:match([[AdvancedInterfaceOptions/(.-)"%]:(%d+)]])
    AdvancedInterfaceOptions.EventSources[ key ] = file and ( file .. ":" .. ( line or 0 ) ) or stack:match( "^(.*)\n" )
end
...
ns.RegisterUnitEvent = function( event, unit1, unit2, handler )
    ...
    if not unitHandlers[ unit1 ] then
        ...
        AdvancedInterfaceOptions:ProfileFrame( "UnitEvents:" .. unit1, unitHandlers[ unit1 ] )
    ...
    local stack = debugstack(2)
    local file, line = stack:match([[AdvancedInterfaceOptions/(.-)"%]:(%d+)]])
    AdvancedInterfaceOptions.EventSources[ event .. "_" .. unit1 .. "_" .. #unitFrame.events[ event ] ] = file and ( file .. ":" .. ( line or 0 ) ) or stack:match( "^(.*)\n" )

    unitFrame:RegisterUnitEvent( event, unit1 )
    AdvancedInterfaceOptions:ProfileCPU( event .. "_" .. unit1 .. "_" .. #unitFrame.events[ event ], handler )

    if unit2 then
        if not unitHandlers[ unit2 ] then
            unitHandlers[ unit2 ] = CreateFrame( "Frame" )
            AdvancedInterfaceOptions:ProfileFrame( "UnitEvents:" .. unit2, unitHandlers[ unit2 ] )
        ...
        AdvancedInterfaceOptions.EventSources[ event .. "_" .. unit2 .. "_" .. #unitFrame.events[ event ] ] = file and ( file .. ":" .. ( line or 0 ) ) or stack:match( "^(.*)\n" )

        unitFrame:RegisterUnitEvent( event, unit2 )
        AdvancedInterfaceOptions:ProfileCPU( event .. "_" .. unit2 .. "_" .. #unitFrame.events[ event ], handler )
    end
end
...
AdvancedInterfaceOptions.FeignEvent = ns.FeignEvent
...
do
    ...
    function AdvancedInterfaceOptions:ContinueOnSpellLoad( spellID, func )
    ...
    function AdvancedInterfaceOptions:RunSpellCallbacks()
...
RegisterEvent( "PLAYER_ENTERING_WORLD", function( event, login, reload )
    if not AdvancedInterfaceOptions.PLAYER_ENTERING_WORLD and ( login or reload ) then
        AdvancedInterfaceOptions.PLAYER_ENTERING_WORLD = true
        AdvancedInterfaceOptions:SpecializationChanged()
        AdvancedInterfaceOptions:RestoreDefaults()
...
do
    if AdvancedInterfaceOptions.IsWrath() then
        ...
        local function CheckForTalentUpdate( event )
            ...
            if specID and specID ~= state.spec.id then
                AdvancedInterfaceOptions.PendingSpecializationChange = true
            end
        end
...
RegisterEvent( "BARBER_SHOP_OPEN", function ()
    AdvancedInterfaceOptions.Barber = true
end )

RegisterEvent( "BARBER_SHOP_CLOSE", function ()
    AdvancedInterfaceOptions.Barber = false
end )


-- Update visibility when getting on/off a taxi.
RegisterEvent( "PLAYER_CONTROL_LOST", function ()
    AdvancedInterfaceOptions:After( 0.1, AdvancedInterfaceOptions.UpdateDisplayVisibility, AdvancedInterfaceOptions )
end )
...
do
                ...
                for tier, tierInfo in ipairs( tiers ) do
                    for _, power in ipairs( tierInfo.azeritePowerIDs ) do
                        ...
                        if IsPowerSelected( loc, power ) then
                            ...
                            if not name then
                                AdvancedInterfaceOptions:Error( "Missing Azerite Power info for #" .. pInfo.spellID .. ": " .. GetSpellInfo( pInfo.spellID ) .. "." )
                            else
    ...
    AdvancedInterfaceOptions:ProfileCPU( "updatePowers", ns.updatePowers )
...
do
    ...
    local function buildUseItemsList()
        ...
        if #state.items > 0 then
            for i, item in ipairs( state.items ) do
                if not AdvancedInterfaceOptions:IsItemScripted( item ) then
                    insert( itemList, {
                        action = item,
                        enabled = true,
        ...
        class.essence_unscripted = ( class.active_essence and not AdvancedInterfaceOptions:IsEssenceScripted( class.active_essence ) ) or false

        AdvancedInterfaceOptions:LoadItemScripts()
    end

    function AdvancedInterfaceOptions:UpdateUseItems()
        if not AdvancedInterfaceOptions.PLAYER_ENTERING_WORLD then
    ...
    function AdvancedInterfaceOptions:RegisterGearHook( r, u )
        insert( GearHooks, {
            reset = r,
            update = u
        } )
    ...
    local wasWearing = {}
    local maxItemSlot = AdvancedInterfaceOptions.IsWrath() and INVSLOT_LAST_EQUIPPED or Enum.ItemSlotFilterTypeMeta.MaxValue
    ...
    function ns.updateGear()
        ...
        AdvancedInterfaceOptions:UpdateUseItems()
...
RegisterEvent( "PLAYER_REGEN_DISABLED", function( event )
    ...
    if AdvancedInterfaceOptions.Config and not LibStub( "AceConfigDialog-3.0" ).OpenFrames[ "AdvancedInterfaceOptions" ] then
        ns.StopConfiguration()
        AdvancedInterfaceOptions:UpdateDisplayVisibility()
    end

    -- AdvancedInterfaceOptions:ExpireTTDs( true )
    AdvancedInterfaceOptions:ForceUpdate( event ) -- Force update on entering combat since OOC refresh can be very slow (0.5s).
end )
...
RegisterEvent( "PLAYER_REGEN_ENABLED", function ()
    ...
    AdvancedInterfaceOptions:ReleaseHolds( true )
    AdvancedInterfaceOptions:UpdateDisplayVisibility()
end )
...
do
    ...
    function AdvancedInterfaceOptions:GetMacroCastTarget( spell, castTime, source )
...
RegisterUnitEvent( "UNIT_SPELLCAST_SUCCEEDED", "player", "target", function( event, unit, _, spellID )
    if not noClassWarned and not class.initialized then
        AdvancedInterfaceOptions:Notify( UnitClass( "player" ) .. " does not have any AdvancedInterfaceOptions modules loaded (yet).\nWatch for updates.", 10 )
        noClassWarned = true
    elseif not lowLevelWarned and UnitLevel( "player" ) < minimumLevel then
        AdvancedInterfaceOptions:Notify( "AdvancedInterfaceOptions is designed for use in " .. expansionName .. " content.\nIt may not work as expected below level " .. minimumLevel .. ".", 10 )
        lowLevelWarned = true
    end
    ...
    if unit == "player" then
        ...
        if ability then
            AdvancedInterfaceOptions:ForceUpdate( event )
            if state.holds[ ability.key ] then AdvancedInterfaceOptions:RemoveHold( ability.key, true ) end
        end
    end
end )
...
RegisterUnitEvent( "UNIT_SPELLCAST_START", "player", "target", function( event, unit, cast, spellID )
    if unit == "player" then
        ...
        AdvancedInterfaceOptions:ForceUpdate( event, true )
    end
end )
...
RegisterUnitEvent( "UNIT_SPELLCAST_DELAYED", "player", nil, function( event, unit, _, spellID )
    ...
    if ability then
        ...
        if start and finish then
            if not target then target = AdvancedInterfaceOptions:GetMacroCastTarget( action, start / 1000, "DELAYED" ) end
            ...
            if ability.isProjectile then
                ...
                elseif target then
                    local u = AdvancedInterfaceOptions:GetUnitByGUID( target ) or AdvancedInterfaceOptions:GetNameplateUnitForGUID( target ) or "target"
...
RegisterEvent( "UNIT_SPELLCAST_SENT", function ( event, unit, target_name, castID, spellID )
    ...
    local gubn = AdvancedInterfaceOptions:GetUnitByName( target_name )
...
AdvancedInterfaceOptions:ProfileCPU( "UNIT_POWER_FREQUENT", UNIT_POWER_FREQUENT )
...
do
    ...
    RegisterUnitEvent( "UNIT_AURA", "player", "target", function( event, unit, data )
        ...
        if data.isFullUpdate then
            ...
            AdvancedInterfaceOptions:ForceUpdate( "UNIT_AURA_FULL", true )
            return
        end
        ...
        if forceUpdateNeeded then AdvancedInterfaceOptions:ForceUpdate( "UNIT_AURA_ITER" ) end
    end )
...
function AdvancedInterfaceOptions:UpdateDamageDetectionForCLEU()
...
-- Note that this was ported from an unreleased version of AdvancedInterfaceOptions, and is currently only counting damaged enemies.
local function CLEU_HANDLER( event, timestamp, subtype, hideCaster, sourceGUID, sourceName, sourceFlags, sourceRaidFlags, destGUID, destName, destFlags, destRaidFlags, spellID, spellName, school, amount, interrupt, a, b, c, d, offhand, multistrike, ... )
    ...
    if death_events[ subtype ] then
        if ns.isTarget( destGUID ) then
            ...
            -- AdvancedInterfaceOptions:ForceUpdate( subtype )
            ...
            if ability then
                if subtype == "SPELL_CAST_START" then
                    ...
                    if destGUID:len() == 0 then
                        destGUID = AdvancedInterfaceOptions:GetMacroCastTarget( ability.key, GetTime(), "START" ) or UnitGUID( "target" )
                    end
                    ...
                    if start then
                        ...
                        if ability.isProjectile then
                            ...
                            elseif destGUID then
                                local unit = AdvancedInterfaceOptions:GetUnitByGUID( destGUID ) or AdvancedInterfaceOptions:GetNameplateUnitForGUID( destGUID ) or "target"
                ...
                elseif subtype == "SPELL_CAST_FAILED" then
                    ...
                    -- AdvancedInterfaceOptions:ForceUpdate( "SPELL_CAST_FAILED" ) ]]
                ...
                elseif subtype == "SPELL_AURA_REMOVED" and ability.channeled then
                    ...
                    -- AdvancedInterfaceOptions:ForceUpdate( "SPELL_AURA_REMOVED_CHANNEL" )
                ...
                elseif subtype == "SPELL_DAMAGE" then
                    ...
                    if state:RemoveSpellEvent( ability.key, true, "PROJECTILE_IMPACT" ) then
                        AdvancedInterfaceOptions:ForceUpdate( "PROJECTILE_IMPACT" )
                    end
                end
            end

            state.gcd.lastStart = max( state.gcd.lastStart, ( GetSpellCooldown( 61304 ) ) )
            -- if subtype ~= "SPELL_DAMAGE" then AdvancedInterfaceOptions:ForceUpdate( subtype, true ) end
        end
    end
    ...
    elseif ( amSource or petSource or isSensePower ) or ( countPets and minion ) or ( sourceGUID == destGUID and sourceGUID == UnitGUID( 'target' ) ) then
        --[[ if aura_events[ subtype ] then
            if subtype == "SPELL_CAST_SUCCESS" or state.GUID == destGUID then
                if class.abilities[ spellID ] or class.auras[ spellID ] then
                    AdvancedInterfaceOptions:ForceUpdate( subtype, true )
                end
            end

            if UnitGUID( 'target' ) == destGUID then
                if class.auras[ spellID ] then AdvancedInterfaceOptions:ForceUpdate( subtype ) end
            end
        end ]]
            ...
            if hostile and sourceGUID ~= destGUID and not aura.friendly then
                ...
                if subtype == 'SPELL_AURA_APPLIED' or subtype == 'SPELL_AURA_REFRESH' or subtype == 'SPELL_AURA_APPLIED_DOSE' then
                    ...
                    --[[ if spellID == 48108 or spellID == 48107 then
                        AdvancedInterfaceOptions:ForceUpdate( "SPELL_AURA_SUPER", true )
                    end ]]
        ...
        if hostile and ( countDots and dmg_events[ subtype ] or direct_dmg_events[ subtype ] ) and not dmg_filtered[ spellID ] then
            ...
            if not IsResting() and ( ( ( subtype == "SPELL_DAMAGE" or subtype == "SPELL_PERIODIC_DAMAGE" ) and interrupt > 0 ) or ( subtype == "SWING_DAMAGE" and spellName > 0 ) ) and ns.isTarget( destGUID ) then
                ns.eliminateUnit( destGUID, true )
                -- AdvancedInterfaceOptions:ForceUpdate( "SPELL_DAMAGE_OVERKILL" )
...
AdvancedInterfaceOptions:ProfileCPU( "CLEU_HANDLER", CLEU_HANDLER )
...
do
    ...
    AdvancedInterfaceOptions:ProfileCPU( "UNIT_COMBAT", UNIT_COMBAT )
    RegisterUnitEvent( "UNIT_COMBAT", "player", nil, UNIT_COMBAT )
end


local keys = ns.hotkeys
AdvancedInterfaceOptions.KeybindInfo = keys
local updatedKeys = {}
...
local function ReadKeybindings( event )
        if not AdvancedInterfaceOptions:IsValidSpec() then return end
        ...
        AdvancedInterfaceOptions:SetupPetBasedTargetDetection()
end
...
if select( 2, UnitClass( "player" ) ) == "DRUID" then
    ...
    function AdvancedInterfaceOptions:GetBindingForAction( key, display, i )
