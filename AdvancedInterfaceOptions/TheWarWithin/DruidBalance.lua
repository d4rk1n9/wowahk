...
local PTR = ns.PTR
local spec = AdvancedInterfaceOptions:NewSpecialization( 102 )
...
do
    ...
    function AdvancedInterfaceOptions:EmbedAdaptiveSwarm( s )
...
local ExitEclipse = setfenv( function()
    ...
    if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Expire Eclipse: %s - Starfire(%d), Wrath(%d), Solar(%.2f), Lunar(%.2f)", eclipse.state, eclipse.starfire_counter, eclipse.wrath_counter, buff.eclipse_solar.remains, buff.eclipse_lunar.remains ) end
end, state )
...
spec:RegisterHook( "step", function()
    if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Eclipse State: %s, Wrath: %d, Starfire: %d; Lunar: %.2f, Solar: %.2f\n", eclipse.state or "NOT SET", eclipse.wrath_counter, eclipse.starfire_counter, buff.eclipse_lunar.remains, buff.eclipse_solar.remains ) end
end )
...
spec:RegisterAbilities( {
    ...
    force_of_nature = {
        ...
        usable = function ()
            return AdvancedInterfaceOptions:safeAOEEnable(205636)
        end,
        handler = function ()
...
spec:RegisterSetting( "vigil_damage", 50, {
    name = strformat( "%s Damage Threshold", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.natures_vigil.id ) ),
...
spec:RegisterSetting( "starlord_cancel", false, {
    name = strformat( "%s |TInterface\\Addons\\AdvancedInterfaceOptions\\Textures\\Cancel:0|t Cancel", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.auras.starlord.id ) ),
    desc = strformat( "If checked, canceling |TInterface\\Addons\\AdvancedInterfaceOptions\\Textures\\Cancel:0|t your %s may be recommended.  Canceling it allows you to start building stacks via %s and %s at its full duration.\n\n"
        .. "You will likely want a |cFFFFD100/cancelaura %s|r macro to manage this during combat.", spec.auras.starlord.name, AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.starsurge.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.starfall.id ), spec.auras.starlord.name ),
    icon = 462651,
...
--[[ spec:RegisterSetting( "delay_berserking", false, {
    name = strformat( "Delay %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( class.specs[ 0 ].auras.berserking.id ) ),
