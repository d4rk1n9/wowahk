...
local spec = AdvancedInterfaceOptions:NewSpecialization( 1467 )
...
spec:RegisterHook( "reset_precast", function()
    ...
    if AdvancedInterfaceOptions.ActiveDebug and active_dot.enkindle > 0 then
        AdvancedInterfaceOptions:Debug( "Enkindles: %d; Target Enkindle: %.2f", active_dot.enkindle, debuff.enkindle.remains )
    end
end )
...
spec:RegisterAbilities( {
    ...
    eternity_surge = {
        ...
        empowerment_default = function()
            ...
            if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Eternity Surge empowerment level, cast time: %.2f, %.2f", n, empowered_cast_time( n ) ) end
            return n
        end,
    ...
    firestorm = {
        ...
        color = "red",
        usable = function ()
            return AdvancedInterfaceOptions:safeAOEEnable(368847)
        end,
        talent = "firestorm",
        startsCombat = true,
...
spec:RegisterSetting( "dragonrage_pad", 0.5, {
    name = strformat( "%s: %s Padding", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.dragonrage.id ), AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.animosity[2] ) ),
    type = "range",
    desc = strformat( "If set above zero, extra time is allotted to help ensure that %s and %s are used before %s expires, reducing the risk that you'll fail to extend "
        .. "it.\n\nIf %s is not talented, this setting is ignored.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.fire_breath.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.eternity_surge.id ), AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.dragonrage.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.animosity[2] ) ),
    min = 0,
    max = 1.5,
    step = 0.05,
    width = "full",
...
spec:RegisterSetting( "use_deep_breath", false, {
    name = strformat( "Use %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.deep_breath.id ) ),
    type = "toggle",
    desc = strformat( "If checked, %s may be recommended, which will force your character to select a destination and move.  By default, %s requires your Cooldowns "
        ...
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.deep_breath.id ), spec.abilities.deep_breath.name, spec.abilities.deep_breath.name ),
    width = "full",
} )

spec:RegisterSetting( "simplify_engulf", true, {
    name = strformat( "Simplify %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.engulf.id ) ),
    type = "toggle",
    desc = strformat( "If checked, the requirements for using %s will be eased, resulting in more %s casts.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.engulf.id ),
        spec.abilities.engulf.name ),
    width = "full"
} )

spec:RegisterSetting( "use_unravel", false, {
    name = strformat( "Use %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.unravel.id ) ),
    type = "toggle",
    desc = strformat( "If checked, %s may be recommended if your target has an absorb shield applied.  By default, %s also requires your Interrupts toggle to be active.",
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.unravel.id ), spec.abilities.unravel.name ),
    width = "full",
} )

spec:RegisterSetting( "fire_breath_fixed", 0, {
    name = strformat( "%s: Empowerment", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.fire_breath.id ) ),
    type = "range",
    desc = strformat( "If set to |cffffd1000|r, %s will be recommended at different empowerment levels based on the action priority list.\n\n"
        ...
        .. "If the selected empowerment level exceeds your maximum, the maximum level will be used instead.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.fire_breath.id ),
...
spec:RegisterSetting( "use_early_chain", false, {
    name = strformat( "%s: Chain Channel", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.disintegrate.id ) ),
    type = "toggle",
    desc = strformat( "If checked, %s may be recommended while already channeling |W%s|w, extending the channel.",
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.disintegrate.id ), spec.abilities.disintegrate.name ),
    width = "full"
} )

spec:RegisterSetting( "use_clipping", false, {
    name = strformat( "%s: Clip Channel", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.disintegrate.id ) ),
    type = "toggle",
    desc = strformat( "If checked, other abilities may be recommended during %s, breaking its channel.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.disintegrate.id ) ),
    width = "full",
} )

spec:RegisterSetting( "use_verdant_embrace", false, {
    name = strformat( "%s: %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.verdant_embrace.id ), AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.ancient_flame[2] ) ),
    type = "toggle",
