...
local spec = AdvancedInterfaceOptions:NewSpecialization( 103 )
    ...
    if action == "primal_wrath" and active_enemies > 1 then
        ...
        for guid, counted in pairs( targets ) do
            if counted then
                ...
                ttd = min( fight_remains, max( 1, AdvancedInterfaceOptions:GetDeathClockByGUID( guid ) - ( offset + delay ) ) )
...
spec:RegisterHook( "reset_precast", function ()
    ...
    if talent.bloodtalons.enabled then
        ...
        if AdvancedInterfaceOptions.ActiveDebug then
            AdvancedInterfaceOptions:Debug( "\n*** |cffffd100Bloodtalons|r Status ***" )

            if buff.bloodtalons.up then
                AdvancedInterfaceOptions:Debug( "    Stacks: %d", buff.bloodtalons.stack )
                AdvancedInterfaceOptions:Debug( "    Remains: %.2f", buff.bloodtalons.remains )
            else
                AdvancedInterfaceOptions:Debug( "    |cffffd100Bloodtalons|r Down" )
            end
            ...
            AdvancedInterfaceOptions:Debug( "    Active BT Buffs: %d", count )

            for bt_buff, spell in pairs( bt_auras ) do
                if type( spell ) == "function" then spell = spell() end
                local bt = bt_buff
                if buff[ bt ] and buff[ bt ].up then
                    AdvancedInterfaceOptions:Debug( "        %s: %.2f", bt, buff[ bt ].remains )
                end
            end
            AdvancedInterfaceOptions:Debug( "\n")
        end
...
spec:RegisterAbilities( {
    ...
    prowl = {
        ...
        usable = function ()
            AdvancedInterfaceOptions:Debug( "Time(%d), Jungle Stalker(%s), Incarnation of Ashamane Prowl(%s)", time, tostring( buff.jungle_stalker.up ), tostring( buff.incarnation_avatar_of_ashamane_prowl.up ) )
...
spec:RegisterSetting( "use_funnel", false, {
    name = strformat( "%s Funnel", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.ferocious_bite.id ) ),
    desc = function()
        return strformat( "If checked, when %s and %s are talented and %s is |cFFFFD100not|r talented, %s will be recommended over %s unless |W%s|w needs to be "
            ...
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.taste_for_blood[2] ), AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.relentless_predator[2] ),
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.tear_open_wounds[2] ), AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.ferocious_bite.id ),
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.primal_wrath.id ), AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.rip.id ),
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.taste_for_blood[2], nil, state.talent.taste_for_blood.enabled ),
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.relentless_predator[2], nil, state.talent.relentless_predator.enabled ),
            ( not state.talent.tear_open_wounds.enabled and "FF00FF00" or "FFFF0000" ),
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.tear_open_wounds[2], nil, not state.talent.tear_open_wounds.enabled ) )
    end,
    type = "toggle",
    width = "full"
} )  ]]
...
spec:RegisterSetting( "zerk_biteweave", false, {
    name = strformat( "%s Biteweave", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.berserk.id ) ),
    desc = function()
        return strformat( "If checked, the default priority will recommend %s more often when %s or %s is active.\n\n"
            .. "This option may not be optimal for all situations; the default setting is unchecked.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.ferocious_bite.id ),
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.berserk.id ), AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.incarnation.id ) )
    end,
    type = "toggle",
    width = "full"
} )
...
spec:RegisterSetting( "rip_duration", 9, {
    name = strformat( "%s Duration", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.rip.id ) ),
    desc = strformat( "If set above |cFFFFD1000|r, %s will not be recommended if the target will die within the specified timeframe.",
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.rip.id ) ),
    type = "range",
    min = 0,
    max = 18,
    step = 0.1,
...
spec:RegisterSetting( "frenzy_cp", 2, {
    name = strformat( "%s: Combo Point Cap", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.feral_frenzy.id ) ),
    desc = strformat( "In the default priority, %s will only be recommended if you have fewer than the specified number of Combo Points. "
        ...
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.feral_frenzy.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.berserk.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.incarnation.id ) ),
    type = "range",
    min = 1,
    max = 5,
    step = 1,
...
spec:RegisterSetting( "vigil_damage", 50, {
    ...
    desc = strformat( "If set below |cFFFFD100100%%|r, %s may only be recommended if your health has dropped below the specified percentage.\n\n"
        ...
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( class.specs[ 102 ].abilities.natures_vigil.id ),
...
spec:RegisterSetting( "allow_crit_prediction", true, {
    name = strformat( "%s Combo Point Prediction", AdvancedInterfaceOptions:GetSpellLinkWithTexture( 159286 ) ), -- Primal Fury
    desc = strformat( "This setting enables prediction of an additional combo point on critical strikes when talented into %s.\n\n" ..
                      ...
                      AdvancedInterfaceOptions:GetSpellLinkWithTexture( 159286 )
    ),
    type = "toggle",
    width = "full",
} )
...
spec:RegisterSetting( "lazy_swipe", false, {
    name = strformat( "%s: Don't %s in AOE", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.wild_slashes[2] ), AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.shred.id ) ),
    desc = function()
        return strformat( "If checked, when %s is talented, the use of %s will be minimized in multi-target situations even if "
            ...
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.wild_slashes[2] ),
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.shred.id ),
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.bloodtalons[2] ) )
    end,
    type = "toggle",
    width = "full"
} )
...
spec:RegisterSetting( "regrowth", true, {
    name = strformat( "Filler %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.regrowth.id ) ),
    desc = strformat( "If checked, %s may be recommended when higher priority abilities are not available or recommended.\n\n"
        ...
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.regrowth.id ) ),
    type = "toggle",
    width = "full",
} )
...
spec:RegisterSetting( "solo_prowl", false, {
    name = strformat( "Allow %s in Combat When Solo", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.prowl.id ) ),
    desc = strformat( "If checked, %s can be recommended in combat when %s is active and you are solo.\n\n"
        ...
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.prowl.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.auras.jungle_stalker.id ),
...
spec:RegisterSetting( "allow_shadowmeld", nil, {
    name = strformat( "Use %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.auras.shadowmeld.id ) ),
