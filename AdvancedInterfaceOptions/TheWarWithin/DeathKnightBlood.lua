...
local PTR = ns.PTR
local spec = AdvancedInterfaceOptions:NewSpecialization( 250 )
...
spec:RegisterHook( "reset_precast", function ()
    ...
    local name, _, count, _, duration2, expires, caster = FindUnitBuffByID( "player", 188290 )
    if buff.death_and_decay.up and duration2 > 4 then
...
spec:RegisterAbilities( {
    ...
    death_and_decay = {
        ...
        usable = function () return ( settings.dnd_while_moving or not moving ) and AdvancedInterfaceOptions:safeAOEEnable(43265), "cannot cast while moving" end,
...
spec:RegisterSetting( "dnd_while_moving", true, {
    name = strformat( "允许释放 %s 在移动中", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.death_and_decay.id ) ),
    desc = strformat( "如果勾选，则允许在玩家移动时释放 %s，否则仅在玩家站立不动时释放。", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.death_and_decay.id ) ),
    type = "toggle",
    width = "full",
} )

spec:RegisterSetting( "save_blood_shield", true, {
    name = strformat( "Save %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.auras.blood_shield.id ) ),
    desc = strformat( "If checked, the default priority (or any priority checking |cFFFFD100save_blood_shield|r) will try to avoid letting your %s fall off during "
        .. "lulls in damage.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.auras.blood_shield.id ) ),
    type = "toggle",
    width = "full"
} )

spec:RegisterSetting( "death_strike_pool_amount", 65, {
    name = strformat( "%s %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.death_strike.id ), _G.POWER_TYPE_RUNIC_POWER ),
    desc = strformat( "The default priority will (usually) avoid spending %s on %s unless you have pooled at least this much.", _G.POWER_TYPE_RUNIC_POWER, AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.death_strike.id ) ),
    type = "range",
    min = 40,
    max = 125,
    step = 1,
...
spec:RegisterSetting( "ibf_damage", 40, {
    name = strformat( "%s Damage Threshold", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.icebound_fortitude.id ) ),
    desc = strformat( "When set above zero, the default priority can recommend %s if you've lost this percentage of your maximum health in the past 5 seconds.\n\n"
        .. "|W%s|w also requires the Defensives toggle by default.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.icebound_fortitude.id ),
...
spec:RegisterSetting( "rt_damage", 30, {
    name = strformat( "%s Damage Threshold", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.rune_tap.id ) ),
    desc = strformat( "When set above zero, the default priority can recommend %s if you've lost this percentage of your maximum health in the past 5 seconds.\n\n"
        .. "|W%s|w also requires the Defensives toggle by default.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.rune_tap.id ), spec.abilities.rune_tap.name ),
    type = "range",
    min = 0,
    max = 200,
    step = 1,
...
spec:RegisterSetting( "vb_damage", 50, {
    name = strformat( "%s Damage Threshold", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.vampiric_blood.id ) ),
    desc = strformat( "When set above zero, the default priority can recommend %s if you've lost this percentage of your maximum health in the past 5 seconds.\n\n"
        .. "|W%s|w also requires the Defensives toggle by default.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.vampiric_blood.id ),
