...
local spec = AdvancedInterfaceOptions:NewSpecialization( 255 )
...
spec:RegisterSetting( "pet_healing", 0, {
    ...
    desc = strformat( "If set above zero, %s will be recommended when your pet falls below this health percentage. Set to 0 to disable the feature.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( beastMastery.abilities.mend_pet.id ) ),
    icon = 132179,
...
spec:RegisterSetting( "use_harpoon", false, {
    name = strformat( "Use %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.harpoon.id ) ),
    desc = strformat( "If checked, %s will be recommended when you are out of range and it is available.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.harpoon.id ) ),
    type = "toggle",
    width = "full"
} )
...
spec:RegisterSetting( "lunar_toggle", "none", {
    name = strformat( "|T2065634:0|t %s: 技能归类", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.lunar_storm[2] ) ),
    desc = strformat(
        "当%s被点选且不在冷却时，%s仅在启用对应开关时才会被推荐。\n\n若已在|cFFFFD100技能与物品|r中设置%s的开关，本设定将被忽略。\n\n选择|cFFFFD100不覆盖|r可关闭此功能。",
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.talents.lunar_storm[2] ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.wildfire_bomb.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.wildfire_bomb.id )
    ),
    type = "select",
    width = 2,
