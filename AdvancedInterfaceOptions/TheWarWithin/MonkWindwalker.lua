...
local spec = AdvancedInterfaceOptions:NewSpecialization( 269 )
    ...
    if sourceGUID == state.GUID then
        ...
        if ability then
            if ability == "tiger_palm" and subtype == "SPELL_MISSED" and not state.talent.hit_combo.enabled then
                ...
                AdvancedInterfaceOptions:ForceUpdate( "WW_MISSED" )
    ...
    blackout_kick = {
        ...
        --[[cycle = function()
            ...
            if level > 32 and cycle_enemies > active_dot.mark_of_the_crane and active_dot.mark_of_the_crane < 5 and debuff.mark_of_the_crane.up then
                if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Recommending swap to target missing Mark of the Crane debuff." ) end
...
spec:RegisterSetting( "allow_fsk", false, {
    name = strformat( "Use %s", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.flying_serpent_kick.id ) ),
    desc = strformat( "If unchecked, %s will not be recommended despite generally being used as a filler ability.\n\n"
            ...
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.flying_serpent_kick.id ), spec.name, spec.abilities.flying_serpent_kick.name ),
    type = "toggle",
    width = "full",
    get = function () return not AdvancedInterfaceOptions.DB.profile.specs[ 269 ].abilities.flying_serpent_kick.disabled end,
    set = function ( _, val )
        AdvancedInterfaceOptions.DB.profile.specs[ 269 ].abilities.flying_serpent_kick.disabled = not val
    end
} )

spec:RegisterSetting( "sef_one_charge", false, {
    name = strformat( "%s: Reserve 1 Charge for Cooldowns Toggle", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.storm_earth_and_fire.id ) ),
    desc = strformat( "If checked, %s can be recommended while Cooldowns are disabled, as long as you will retain 1 remaining charge.\n\n"
            ...
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.storm_earth_and_fire.id ), spec.abilities.storm_earth_and_fire.name ),
    type = "toggle",
    width = "full"
} )

spec:RegisterSetting( "dynamic_strike_of_the_windlord", false, {
    name = strformat( "%s: Raid Cooldown", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.strike_of_the_windlord.id ) ),
    desc = strformat(
        ...
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.strike_of_the_windlord.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.strike_of_the_windlord.id )
    ),
    type = "toggle",
    width = "full"
...
spec:RegisterSetting( "cjl_capacitor_toggle", "none", {
    name = strformat( "%s: Special Toggle", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.crackling_jade_lightning.id ) ),
    desc = strformat(
        ...
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.auras.the_emperors_capacitor.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.crackling_jade_lightning.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.crackling_jade_lightning.id )
    ),
    type = "select",
    width = 2,
...
spec:RegisterSetting( "check_wdp_range", false, {
    name = strformat( "%s: Check Range", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.whirling_dragon_punch.id ) ),
    desc = strformat( "If checked, %s will not be recommended if your target is outside your %s range.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.whirling_dragon_punch.id ), AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.fists_of_fury.id ) ),
    type = "toggle",
    width = "full"
} )

spec:RegisterSetting( "check_sck_range", false, {
    name = strformat( "%s: Check Range", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.spinning_crane_kick.id ) ),
    desc = strformat( "If checked, %s will not be recommended if your target is outside your %s range.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.spinning_crane_kick.id ), AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.fists_of_fury.id ) ),
    type = "toggle",
    width = "full"
} )

spec:RegisterSetting( "use_diffuse", false, {
    name = strformat( "%s: Self-Dispel", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.diffuse_magic.id ) ),
    desc = function()
        local m = strformat( "If checked, %s may be recommended when when you have a dispellable magic debuff.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.diffuse_magic.id ) )

        local t = class.abilities.diffuse_magic.toggle
        if t then
            local active = AdvancedInterfaceOptions.DB.profile.toggles[ t ].value
...
spec:RegisterSetting( "tok_damage", 1, {
    name = strformat( "%s: Required Incoming Damage", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.touch_of_karma.id ) ),
    desc = strformat( "If set above zero, %s will only be recommended if you have taken this percentage of your maximum health in damage in the past 3 seconds.",
            AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.touch_of_karma.id ) ),
    type = "range",
    min = 0,
    max = 99,
    step = 0.1,
