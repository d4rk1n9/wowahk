...
local spec = AdvancedInterfaceOptions:NewSpecialization( 256 )
...
spec:RegisterAbilities( {
    ...
    mindbender = {
        id = function() return talent.mindbender.enabled and 200174 or 123040 end,
        known = 34433,
    ...
    pain_suppression = {
        ...
        usable = function()
            ...
            local unt = state.find.lowest_hp.unti ~= "none" and state.find.lowest_hp.unti
            if not unt then return false end

            local targetIsFriend = UnitIsFriend(unt, "player")
            local hasHot = false
            hasHot = AdvancedInterfaceOptions:getUntiHasBuff(33206, unt, true)

            if not hasHot and targetIsFriend then
                return true
            end
            return false
        end,
