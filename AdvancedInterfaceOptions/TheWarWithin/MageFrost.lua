...
local spec = AdvancedInterfaceOptions:NewSpecialization( 64 )
...
spec:RegisterStateExpr( "remaining_winters_chill", function ()
    ...
    if stacks == 0 or remains == 0 then
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Remaining Winters Chill(%s): No stacks or remaining time.", this_action ) end
        return 0
    end
    ...
    if remains < cast then
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Remaining Winters Chill(%s): No time to cast.", this_action ) end
        return 0
    end

    if stacks > 1 and remains < 0.1 + cast + gcd.max then
        if AdvancedInterfaceOptions.ActiveDebug then
            AdvancedInterfaceOptions:Debug( "Remaining Winters Chill(%s): Stacks reduced as cast time + GCD (%.2f) exceeds remaining time (%.2f).", this_action, 0.1 + cast + gcd.max, remains )
        end
        stacks = stacks - 1
    end
    ...
    if action.flurry.in_flight and action.flurry.lastCast > 0 then
        ...
        if timeSinceCast < impact1Time then
            ...
            if AdvancedInterfaceOptions.ActiveDebug then
                AdvancedInterfaceOptions:Debug( "Remaining Winters Chill(%s): Flurry projectile 1 still incoming (%.3fs remaining)", this_action, impact1Time - timeSinceCast )
            end
        end
        ...
        if timeSinceCast < impact2Time then
            projectiles_still_incoming = projectiles_still_incoming + 1
            if AdvancedInterfaceOptions.ActiveDebug then
                AdvancedInterfaceOptions:Debug( "Remaining Winters Chill(%s): Flurry projectile 2 still incoming (%.3fs remaining)", this_action, impact2Time - timeSinceCast )
            end
        end
        ...
        if timeSinceCast < impact3Time then
            projectiles_still_incoming = projectiles_still_incoming + 1
            if AdvancedInterfaceOptions.ActiveDebug then
                AdvancedInterfaceOptions:Debug( "Remaining Winters Chill(%s): Flurry projectile 3 still incoming (%.3fs remaining)", this_action, impact3Time - timeSinceCast )
            end
        end
        ...
        if remaining_flight_time < remains then
            ...
            if AdvancedInterfaceOptions.ActiveDebug then
                AdvancedInterfaceOptions:Debug( "Remaining Winters Chill(%s): +%d stacks from %d incoming Flurry projectiles", this_action, incoming_wc_stacks, projectiles_still_incoming )
            end
        else
            if AdvancedInterfaceOptions.ActiveDebug then
                AdvancedInterfaceOptions:Debug( "Remaining Winters Chill(%s): Flurry projectiles won't land before Winter's Chill expires", this_action )
            end
        end
    end
    ...
    for spender in pairs( wc_spenders ) do
        ...
        if in_flight_remains > 0 and in_flight_remains < remains then
            if spender == "ice_lance" and fof_consumed > in_flight_remains then
                if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Remaining Winters Chill(%s): Ice Lance in flight, but FoF consumed before it hits.", this_action ) end
            else
                if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Remaining Winters Chill(%s): -%s projectile will consume stack.", this_action, spender ) end
    ...
    if AdvancedInterfaceOptions.ActiveDebug then
        AdvancedInterfaceOptions:Debug( "Remaining Winter's Chill(%s): Current=%d stacks/%.2fs, Incoming=+%d, Consumers=-%d, Available=%d",
...
spec:RegisterHook( "COMBAT_LOG_EVENT_UNFILTERED", function( _, subtype, _, sourceGUID, sourceName, _, _, destGUID, destName, destFlags, _, spellID, spellName )
    if sourceGUID == GUID then
        ...
        if state.talent.glacial_spike.enabled and ( spellID == 205473 or spellID == 199844 ) and auraChanged[ subtype ] then
            AdvancedInterfaceOptions:ForceUpdate( "ICICLES_CHANGED", true )
        end
...
spec:RegisterCombatLogEvent( function( _, subtype, _, sourceGUID, _, _, _, targetGUID, _, _, _, spellID, _, _, _, amount )
    ...
    if spellID == 44614 and subtype == "SPELL_CAST_SUCCESS" then
        ...
        if AdvancedInterfaceOptions.ActiveDebug then
            AdvancedInterfaceOptions:Debug("Flurry cast: Distance=%.1f, GCD=%.3f, Impacts at %.3f/%.3f/%.3f",
    ...
    elseif spellID == 228354 and ( subtype == "SPELL_DAMAGE" or subtype == "SPELL_MISSED" ) then
        ...
        if castData then
            ...
            if AdvancedInterfaceOptions.ActiveDebug then
                AdvancedInterfaceOptions:Debug( "Flurry impact %d: %.3fs after cast ( predicted: %.3f )",
    ...
    reset = setfenv( function ()
        if talent.incanters_flow.enabled then
            if not incanters_flow.fRegistered then
                AdvancedInterfaceOptions:ProfileFrame( "Incanters_Flow_Frost", incanters_flow.f )
                -- One-time setup.
...
spec:RegisterHook( "reset_precast", function ()
    ...
    if fof_remains > 0 then
        ...
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Applied fof_consumed for %.2f seconds with %d stacks.", fof_remains, numLances ) end
    end

    if numFingers > 0 then
        removeStack( "fingers_of_frost", numFingers )
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Removed %d stacks of fingers_of_frost due to buffed ice_lances in flight.", numFingers ) end
    end

    if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Buffed Ice Lances in-flight: %d, FoF Consumed stacks: %d, FoF Consumed Remains: %.2f, Remaining FoF: %d", numLances, buff.fof_consumed.stack, buff.fof_consumed.remains, buff.fingers_of_frost.stack ) end

    if action.flurry.in_flight then
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Flurry is in-flight, keep Winter's Chill stacks at 2." ) end
    ...
    if AdvancedInterfaceOptions.ActiveDebug then spec.stateExprs.remaining_winters_chill() end
end )
...
AdvancedInterfaceOptions:EmbedDisciplinaryCommand( spec )
...
spec:RegisterAbilities( {
    ...
    flurry = {
        ...
        handler = function ()
            ...
            if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Flurry handler - queueing virtual projectiles." ) end
        ...
        impact = function()
            ...
            if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Flurry projectile impact - adding Winter's Chill stack." ) end
    ...
    ice_lance = {
        ...
        impact = function ()
            ...
            if buff.fof_consumed.up then
                if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Fingers of Frost consumed by Ice Lance." ) end
                removeStack( "fof_consumed" )
            else
                if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Winter's Chill consumed by Ice Lance." ) end
                -- Always consume Winter's Chill if available (Flurry projectiles apply stacks progressively)
                removeDebuffStack( "target", "winters_chill" )
            end
        end,
...
spec:RegisterSetting( "prevent_hardcasts", false, {
    name = strformat( "%s, %s, %s: Instant-Only When Moving",
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.blizzard.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.glacial_spike.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.frostbolt.id )
    ),
    desc = strformat( "If checked, non-instant %s, %s, and %s casts will not be recommended while you are moving.\n\nAn exception is made if %s is talented and active, and your cast " ..
        ...
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.glacial_spike.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.frostbolt.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( 108839 ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( 108839 )
    ),
    type = "toggle",
    width = "full"
...
spec:RegisterSetting( "check_cone_range", true, {
    name = strformat( "%s: Range Check", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.cone_of_cold.id ) ),
