...
local spec = AdvancedInterfaceOptions:NewSpecialization( 70 )
...
spec:RegisterCombatLogEvent( function( _, subtype, _,  sourceGUID, sourceName, _, _, destGUID, destName, destFlags, _, spellID, spellName )
    if sourceGUID == state.GUID then
        ...
        elseif spellID == 408385 then
            ...
            if now - last_crusading_strike > 0.5 then -- Crusader Strikes: Swing Damage
                ...
                if current_crusading_strikes < 2 then
                    AdvancedInterfaceOptions:ForceUpdate( "CRUSADING_STRIKES", true )
                end
            end
        ...
        elseif spellID == 433732 then
            ...
            willBeFree = false
            AdvancedInterfaceOptions:ForceUpdate( "HAMMER_OF_LIGHT_FREE_CAST_APPLIED", true )
        elseif subtype == "SPELL_CAST_SUCCESS" and state.talent.lights_deliverance.enabled and empyreanHammerCallers[ spellID ] and state.talent.hammerfall.enabled then
            ...
            if willBeFree then AdvancedInterfaceOptions:ForceUpdate( "HAMMER_OF_LIGHT_50_LD_STACK_SOON", true ) end
        elseif spellID == 433674 and ( subtype == "SPELL_AURA_APPLIED_DOSE" or subtype == "SPELL_AURA_APPLIED" ) then
            ...
            AdvancedInterfaceOptions:ForceUpdate( "HAMMER_OF_LIGHT_50_LD_STACK_NOW", true )
        end
        -- Gets its own block because it's also in empyreanHammerCallers
        if spellID == 427453 and freeHOLApplied > 0 then
            freeHOLApplied = 0
            AdvancedInterfaceOptions:ForceUpdate( "HAMMER_OF_LIGHT_CAST", true )
        end
    end
end )
...
local CrusadingStrikes = setfenv( function()
    if not action.rebuke.in_range then
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Crusading Strikes energize fails: Out of range." ) end
        return
    end
...
local csStartCombat = setfenv( function()
    ...
    if not action.rebuke.in_range then
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Unable to forecast Crusading Strikes; out of range." ) end
        return
    end
    ...
    if current_crusading_strikes < 2 then
        ...
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "First Crusading Strikes resource gain forecasted for next swing." ) end
        ...
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Immediate Crusading Strikes resource gain on virtual combat start." ) end
        spec.abilities.crusader_strike.handler()
    end
...
spec:RegisterHook( "reset_precast", function ()
    ...
    if time > 0 and talent.crusading_strikes.enabled then
        if not action.rebuke.in_range then
            ...
            if last_crusading_strike == 0 or now - last_crusading_strike > mh_speed then
                if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Unable to forecast Crusading Strikes swing; no prior swings have been detected or the last swing was more than 1 swing timer ago." ) end
            else
                ...
                if AdvancedInterfaceOptions.ActiveDebug then
                    if last_crusading_strike == 0 then AdvancedInterfaceOptions:Debug( "No prior Crusading Strikes swings have been detected; assuming first swing is non-productive." )
                    else AdvancedInterfaceOptions:Debug( "Last Crusading Strikes swing was %.2f seconds ago (vs. %.2f swing timer); it was %s.", time_since, mh_speed, was_productive and "productive" or "non-productive" ) end
                end
    ...
    if AdvancedInterfaceOptions.ActiveDebug then
        AdvancedInterfaceOptions:Debug( "Hammer of Light - freeHOLApplied: %.2f, willBeFree: %s, hol_is_free: %s, hol_will_be_free: %s, buff.hammer_of_light_ready.stack: %d, set_bonus.tww3: %d, buff.lights_deliverance.stack: %d, action.wake_of_ashes.time_since: %.2f",
            freeHOLApplied or 0,
