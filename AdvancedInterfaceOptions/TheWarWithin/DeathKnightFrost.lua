...
local PTR = ns.PTR
local spec = AdvancedInterfaceOptions:NewSpecialization( 251 )
...
spec:RegisterHook( "reset_precast", function ()
    ...
    if PendingKillingMachine > 0 then
        if PendingKillingMachine > now then
            ...
            if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Applied virtual KM.", now ) end
        else
            PendingKillingMachine = 0
            if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Virtual KM expired.", now ) end
        end
    end

    if PendingObliteration > 0 then
        applyBuff( "obliteration", nil, PendingObliteration - now )
        if AdvancedInterfaceOptions.ActiveDebug then AdvancedInterfaceOptions:Debug( "Applied virtual 'Obliteration' (%.2f) discount for Oblit / Frostscythe.", buff.obliteration.remains ) end
    end
end )
...
spec:RegisterSetting( "dnd_while_moving", true, {
    name = strformat( "Allow %s while moving", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.death_and_decay.id ) ),
    desc = strformat( "If checked, then allow recommending %s while the player is moving otherwise only recommend it if the player is standing still.", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.death_and_decay.id ) ),
    type = "toggle",
    width = "full",
} )

spec:RegisterSetting( "ams_usage", "none", {
    name = strformat( "%s Requirements", AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.antimagic_shell.id ) ),
