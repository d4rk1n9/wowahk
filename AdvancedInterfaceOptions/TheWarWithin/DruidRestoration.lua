...
local spec = AdvancedInterfaceOptions:NewSpecialization( 105 )
...
spec:RegisterAbilities( {
    ...
    tranquility = {
        ...
        start = function()
            ...
            for i = 1, 4 do
                tickTime = tickTime + haste
                if tickTime <= query_time + (5 * haste) then
...
spec:RegisterSetting( "experimental_msg", nil, {
    ...
    name = strformat( "Restoration Druid supports healing maintenance by recommending key abilities. It will suggest maintaining %s, keeping at least one %s active, maintaining %s, using %s after a %s, and alerting you when %s can activate %s.",
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.lifebloom.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.rejuvenation.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.efflorescence.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.wild_growth.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.swiftmend.id ),
        AdvancedInterfaceOptions:GetSpellLinkWithTexture( spec.abilities.swiftmend.id ),
