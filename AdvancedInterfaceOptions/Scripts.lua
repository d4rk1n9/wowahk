...
local function forgetMeNots( str )
   ...
   while( str:find("!") ) do
      ...
      i = i + 1
      if i >= 100 then AdvancedInterfaceOptions:Debug( "无法将字符串 [%s] 中的 '!' 转换为 'not'。", str ); break end
   end
...
local function atToAbs( str )
    ...
    while( str:find("@") ) do
       ...
       i = i + 1
       if i >= 100 then AdvancedInterfaceOptions:Error( "Was unable to convert '@' to 'abs' in string [%s].", str ); break end
    end
        ...
        if #exprs > 0 then
            for i, expr in ipairs( exprs ) do
                ...
                if converted then
                    ...
                    local rslt, msg = AdvancedInterfaceOptions:Loadstring( "return " .. calc )
                    if not rslt then
                        AdvancedInterfaceOptions:Debug( "Recheck failed to loadstring: %s", msg )
                    else
            ...
            if piece and piece.t == "expr" then
                ...
                if ( prev and prev.t == "op" and math_ops[ prev.a ] and not equality[ prev.a ] ) or ( next and next.t == "op" and math_ops[ next.a ] and not equality[ next.a ] ) then
                    ...
                    if piece.s:find("^variable%.") then
                        ...
                        local func, warn = AdvancedInterfaceOptions:Loadstring( "return " .. ( SimToLua( piece.s ) or "" ) )
                        if func then
                            setfenv( func, state )
                            ...
                            if not pass and not piece.s:match("variable") then
                                ...
                                AdvancedInterfaceOptions:Error( "无法编译 '" .. safepiece:gsub("%%", "%%%%") .. "' - " .. val .. " (pcall-n)\n\n来自: " .. esString:gsub( "%%", "%%%%" ) )
                            else
                            ...
                            AdvancedInterfaceOptions:Error( "无法编译 '" .. ( piece.s ):gsub("%%","%%%%") .. "' - " .. warn .. " (loadstring-n)\n\n来自: " .. treeToString )
                        end
                    end
                    piece.r = nil
                ...
                elseif not numeric and ( not prev or ( prev.t == "op" and not math_ops[ prev.a ] and not equality[ prev.a ] ) ) and ( not next or ( next.t == "op" and not math_ops[ next.a ] and not equality[ next.a ] ) ) then
                    ...
                    if piece.s:find("^variable") then
                        ...
                        if func then
                            ...
                            if not pass and not piece.s:match("variable") then
                                local safepiece = piece.s:gsub( "%%", "%%%%" )
                                AdvancedInterfaceOptions:Error( "无法编译 '" .. safepiece:gsub("%%", "%%%%") .. "' - " .. val .. " (pcall-b)\n来源: " .. esString:gsub( "%%", "%%%%" ) )                            else
                                if trimmed_prefix ~= "safebool" and ( val == nil or type( val ) == "number" ) then
                                    piece.s = "safebool(" .. piece.s .. ")"
                                end
                            end
                        else
                            AdvancedInterfaceOptions:Error( "无法编译 '" .. ( piece.s ):gsub("%%","%%%%") .. "' - " .. warn .. " (loadstring-b)." )                        end
                    end
                    piece.r = nil
                end
            end
...
local function GetScriptElements( script )
    ...
    for s in c:gmatch( "[^ ,]+" ) do
        ...
        if not e[ s ] and not tonumber( s ) then
            local ef = AdvancedInterfaceOptions:Loadstring( "return ".. s )
            if ef then
                setfenv( ef, state )
...
local function generateDebugPrint( node, condition, header, isRecheck )
    ...
    if argn > 0 then
        ...
        debugPrint, formatArgs = AdvancedInterfaceOptions:Loadstring( generateDebug )

        if formatArgs then
            AdvancedInterfaceOptions:Error( "无法生成 " .. header .. " 的调试打印: " .. formatArgs:gsub( "%%", "%%%%" ) .. "\n" .. generateDebug:gsub( "%%", "%%%%" ) )            return
        end
...
local function ConvertScript( node, hasModifiers, header )
    ...
    if t then sf, e = AdvancedInterfaceOptions:Loadstring( "-- " .. header .. "\nreturn safebool( " .. t .. " )" ) end
    ...
    if hasCriteria or hasValue or hasValueElse then
        ...
        if rs then
            rc, erc = AdvancedInterfaceOptions:Loadstring( "-- " .. header .. " recheck\nreturn " .. rs )
            if rc then setfenv( rc, state ) end

            rEle = GetScriptElements( rs )

            if type( rc ) ~= "function" then
                AdvancedInterfaceOptions:Error( "重新检查函数 " .. clean .. " ( " .. ( rs or "nil" ) .. ") 未成功。" )
                rc = nil
            end
        end
    end
    ...
    if hasModifiers then
        for m, value in pairs( newModifiers ) do
            if node[ m ] then
                ...
                if emulated then
                    sf, e = AdvancedInterfaceOptions:Loadstring( "return " .. emulated )
                end
                ...
                if sf then
                    ...
                    modifiers[ m ] = nil
                    modifiers[ "error:" .. m ] = e .. "\n - " .. emulated
                    AdvancedInterfaceOptions:Error( format( "Error in %s modifier %s: %s\n\n - %s", header, m, e, emulated ) )
                end
            end
        end
        ...
        --[[ if modChecks then
            ...
            if mcString then
                local mcTimes, emsg = AdvancedInterfaceOptions:Loadstring( "-- var " .. header .. " mod recheck\n return " .. mcString )
                if mcTimes then setfenv( mcTimes, state ) end

                if type( mcTimes ) ~= "function" then
                    AdvancedInterfaceOptions:Error( "Modifier recheck function for " .. modChecks .. " ( " .. ( mcString or "nil" ) .. ") was unsuccessful somehow." )
                    mcTimes = nil
                end
...
local function scriptLoader()
    if not AdvancedInterfaceOptions.LoadingScripts and not scriptsLoaded then scripts:LoadScripts() end
end

function AdvancedInterfaceOptions:ScriptsLoaded()
...
function scripts:LoadScripts()
    ...
    AdvancedInterfaceOptions.LoadingScripts = true
        ...
        if specData then
            ...
            for list, lData in pairs( pData.lists ) do
                for action, _ in ipairs( lData ) do
                    AdvancedInterfaceOptions:LoadScript( pack, list, action )
                end
            end
        end
    end

    AdvancedInterfaceOptions.LoadingScripts = false
    scriptsLoaded = true
end


function AdvancedInterfaceOptions:LoadScripts()
...
function AdvancedInterfaceOptions:IsEssenceScripted( token )
...
function AdvancedInterfaceOptions:IsItemScripted( token, specific )
    local pack = AdvancedInterfaceOptions:GetActivePack()
    if not pack then return false end
...
function AdvancedInterfaceOptions.Scripts:LoadItemScripts()
...
function AdvancedInterfaceOptions:LoadItemScripts()
    self.Scripts:LoadItemScripts()
end


function AdvancedInterfaceOptions:LoadScript( pack, list, id )
    ...
    if script.Error then
        AdvancedInterfaceOptions:Error( "Error in " .. scriptID .. " conditions:  " .. ( script.rs or "null" ) .. "\n\n" .. script.Error )
    end
...
local function embedConditionsAndValues( source, elements )
    if source and source ~= "" then
        local wasDebugging = AdvancedInterfaceOptions.ActiveDebug
        ...
        if wasDebugging then AdvancedInterfaceOptions.ActiveDebug = true end
        return source
    end
...
do
    ...
    function scripts:GetConditionsAndValues( scriptID, listName, actID, recheck )
        if troubleshootingSnapshotTimes or not AdvancedInterfaceOptions.ActiveDebug then return "[no data]" end
...
AdvancedInterfaceOptions.dumpKeyCache = key_cache
